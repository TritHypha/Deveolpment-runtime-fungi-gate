...
const CORPUS = [
  ...
  { id: "recognised-unused", src: "secure flow s() -> Int\ncontract { effects { database.write } } { return 1 }",
    job: "liveness control — 007 from BOTH sides (proves the twin lane emits, so E1's suppression is a choice, not a dead lane); database.write avoids the separate audit-evidence diagnostic" },
  { id: "declares-and-uses", src: "secure flow s() -> Int\ncontract { effects { database.write } } { dbWrite() return 1 }",
    job: "SAME-CODE-DIFFERENT-CAUSE exhibit — both emit 007, the twin from its DECL charter, Stage-A because it never observed dbWrite() (E3's vocabulary gap). Reads as a match; a set diff cannot see cause. Kept as the documented limit of v1." },
  { id: "undeclared-use", src: "secure flow s() -> Int\ncontract { effects { database.read } } { dbWrite() return 1 }",
    job: "E3 witness — twin BODY lane derives database.write from dbWrite() and flags the undeclared use (001); Stage-A's observer does not know the name and misses it. The independent database.read declaration keeps the expected shared 007 liveness control without invoking audit-specific policy." },
