...
function toolchainBlobs(captured, pin) {
  ...
  const sourceGetDescriptor = safeObjectGetOwnPropertyDescriptor(captured.sourceBlobs, 'get');
  if (!sourceGetDescriptor || !safeObjectHasOwn(sourceGetDescriptor, 'value') ||
      typeof sourceGetDescriptor.value !== 'function') {
    refuse('SOURCE_ORIGIN_EXPORT_TOOLCHAIN');
  }
  ...
  for (let index = 0; index < localLocators.length; index += 1) {
    ...
    const bytes = callIntrinsic(sourceGetDescriptor.value, captured.sourceBlobs, [locator]);
