...
const GENERATOR_CHECK = process.argv.includes("--generator-check");
const CHECK_GENERATED = CHECK || GENERATOR_CHECK;
...
packages.sort((a, b) => (KIND_ORDER.indexOf(a.kind) - KIND_ORDER.indexOf(b.kind)) || stableCompare(a.name, b.name));
...
tools.sort((a, b) => stableCompare(a.category, b.category) || stableCompare(a.name, b.name));
...
md.push(`**${t.packages}** packages (${t.testBearing} test-bearing · ${formatInteger(t.tests)} tests) · ` +
...
for (const k of KIND_ORDER.concat(Object.keys(byKind).filter((x) => !KIND_ORDER.includes(x)))) {
  ...
  md.push(`| ${k} | ${ps.length} | ${formatInteger(tt)} | ${ps.map((p) => p.name.replace(/^galerina-/, "")).join(", ")} |`);
}
md.push("");
...
const generatedOutputs = new Map([
  [join(OUT_DIR, "INDEX.md"), md.join("\n")],
  [join(OUT_DIR, "index.json"), JSON.stringify(summary, null, 2)],
  [join(OUT_DIR, "packages.mmd"), mm.join("\n")],
  [join(OUT_DIR, "tooling.mmd"), tm.join("\n")],
  [
    join(OUT_DIR, "provenance.json"),
    JSON.stringify(provenance("dev-tool-index", ROOT), null, 2) + "\n",
  ],
]);
...
if (CHECK) {
  ...
  if (problems.length) {
    console.error("dev-tool-index --check FAILED:\n  " + problems.join("\n  "));
    process.exitCode = 1;
  } else if (!JSON_OUT) {
    console.log("dev-tool-index --check: OK");
  }
} else if (GENERATOR_CHECK) {
  if (artifactProblems.length) {
    console.error("dev-tool-index --generator-check FAILED:\n  " + artifactProblems.join("\n  "));
    process.exitCode = 1;
  } else if (!JSON_OUT) {
    console.log("dev-tool-index --generator-check: OK");
  }
}
