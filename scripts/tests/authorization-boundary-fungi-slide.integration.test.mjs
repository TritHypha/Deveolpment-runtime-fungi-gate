...
import { authorize } from "../../packages-galerina/galerina-tower-citizen/dist/index.js";
...
const VALUES = Object.freeze([-1, 0, 1]);
...
it(
  "publishes and independently re-admits Tower-Citizen authorization through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          ...
          sourceFlowName: "authorizeVerdict",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-authorize-slide-"));
    ...
    try {
      ...
      for (const candidateVerdict of VALUES) {
        ...
        assert.equal(verified.value, authorize(candidateVerdict));
        assert.equal(verified.authorityReleased, false);
        if (candidateVerdict !== 1) assert.equal(verified.value, false);
      }
