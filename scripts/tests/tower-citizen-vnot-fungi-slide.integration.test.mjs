...
describe("Slice 90 Tower-Citizen vNot through physical SLIDE/VOK", () => {
  it("publishes, re-admits and executes the complete typed K3 NOT table", {
    ...
    assert.deepEqual(compiled.exports[0].parameterTypeIds, [slide.SAFE_VALUE_TYPE_IDS.verdict]);
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-slice-90-vnot-"));
    ...
    try {
      ...
      let retainedReceipt;
      for (const [input, expected] of K3_NOT) {
        ...
        assert.equal(verified.value, expected, `vNotVerdict(${input})`);
