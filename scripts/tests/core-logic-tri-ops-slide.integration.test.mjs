...
import { triAnd, triNor, triNot, triOr } from "../../packages-galerina/galerina-core-logic/dist/index.js";
...
it(
  "publishes, re-admits, executes and mutation-checks the exact Tri Fungi package through SLIDE/VOK",
  ...
  async () => {
    ...
    const compiled = slide.compileCheckedFungiPackageSet({
      packages: [{
        identity: "@galerina/core-logic",
        version: "1.0.0-beta.2",
        exports: OPERATIONS.map(([name]) => ({
          name,
          sourceFlowName: name,
          sourceBytes,
        })),
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-tri-ops-slide-"));
    ...
    try {
      ...
      for (const [name, reference, arity] of OPERATIONS) {
        const vectors = arity === 1
          ? TRITS.map((left) => [left])
          : TRITS.flatMap((left) => TRITS.map((right) => [left, right]));
        for (const vector of vectors) {
          ...
          assert.equal(verified.value, reference(...vector), `${name}(${vector.join(",")})`);
        }

        const invalidPrepared = await slide.prepareCheckedFungiPackagePublication({
          publicationDirectory,
          packageIdentity: "@galerina/core-logic",
          exportName: name,
          context,
        ...
        assert.equal(invalidPrepared.verdict, 1, JSON.stringify(invalidPrepared));
        const invalidVector = arity === 1 ? [2] : [1, 2];
        const refused = slide.executeTypedCheckedFungiPackagePublication(
          invalidPrepared.packageExecutionHandle,
          invalidVector,
          { steps: 128 },
        );
