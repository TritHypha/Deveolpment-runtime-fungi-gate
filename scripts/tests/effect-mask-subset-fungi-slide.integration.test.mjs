...
const SOURCE = join(
  ...
  "src",
  "self-hosted",
  "effect-mask-subset.fungi",
);
const REGISTRY_SET_ID = "slide.registry.executable-gir.v2c-bitwise-and.v1";
const REGISTRY_SET_DIGEST = "361f086de7b88928cde0b49c02ce480669192f16e3494353e9e82a2962a40a8c";
...
const VECTORS = Object.freeze([
  ...
  Object.freeze([EffectFlags.DatabaseWrite, EffectFlags.DatabaseWrite | EffectFlags.AuditWrite]),
  Object.freeze([EffectFlags.DatabaseWrite, EffectFlags.AuditWrite]),
  Object.freeze([EffectFlags.DatabaseWrite | EffectFlags.AuditWrite, EffectFlags.DatabaseWrite]),
  ...
  Object.freeze([EffectFlags.UnmappedEffect, EffectFlags.UnmappedEffect | EffectFlags.DatabaseRead]),
  Object.freeze([-1, -1]),
  Object.freeze([-1, 0]),
...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    exportName: "effectsSubsetFungi",
    context,
    gates: ALL_ALLOW,
  });
  assert.equal(prepared.verdict, 1, JSON.stringify(prepared));
  assert.equal(prepared.registrySetId, REGISTRY_SET_ID, JSON.stringify(prepared));
  assert.equal(prepared.registrySetDigest, REGISTRY_SET_DIGEST, JSON.stringify(prepared));
  return prepared.packageExecutionHandle;
}

it(
  "publishes and independently re-admits effect-mask subset through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          name: "effectsSubsetFungi",
          sourceFlowName: "effectsSubsetFungi",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-effect-mask-subset-slide-"));
    ...
    try {
      ...
      for (const [required, declared] of VECTORS) {
        ...
        assert.equal(verified.value, effectsSubset(required, declared));
