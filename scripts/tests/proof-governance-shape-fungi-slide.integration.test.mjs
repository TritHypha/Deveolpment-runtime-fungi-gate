...
import { sharesGovernanceShape } from "../../packages-galerina/galerina-core-compiler/dist/index.js";
...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    exportName: "sharesGovernanceShapeFungi",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits governance-shape equality through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          name: "sharesGovernanceShapeFungi",
          sourceFlowName: "sharesGovernanceShapeFungi",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-governance-shape-slide-"));
    ...
    try {
      ...
      for (const [left, right] of VECTORS) {
        ...
        assert.equal(
          verified.value,
          sharesGovernanceShape({ signatureHash: left }, { signatureHash: right }),
        );
      ...
      for (const [field, replacement] of [
        ["schema", "slide.checked-fungi.physical-execution.receipt.v3-text-contains"],
