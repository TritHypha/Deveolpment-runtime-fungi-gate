...
import { stricterResidency } from "../../packages-galerina/galerina-core-compiler/dist/index.js";
...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    exportName: "stricterResidencyFungi",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits residency tightening through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          name: "stricterResidencyFungi",
          sourceFlowName: "stricterResidencyFungi",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-residency-tighten-slide-"));
    ...
    try {
      ...
      const pairs = [
        ...TIERS.flatMap((left) => TIERS.map((right) => [left, right, stricterResidency(left, right)])),
        ...HOSTILE_PAIRS.map(([left, right]) => [left, right, "register_only"]),
      ];
