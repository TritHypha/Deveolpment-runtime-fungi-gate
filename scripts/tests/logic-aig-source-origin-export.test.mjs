...
function installPostImportMapGetPoison() {
  const descriptor = safeObjectGetOwnPropertyDescriptor(Map.prototype, 'get');
  let traps = 0;
  safeObjectDefineProperty(Map.prototype, 'get', {
    configurable: true,
    value() {
      traps += 1;
      throw new Error('ATTACKER_MAP_GET');
    },
  });
  return {
    get traps() { return traps; },
    restore() { safeObjectDefineProperty(Map.prototype, 'get', descriptor); },
  };
}
...
test('Task 6B Map poison observes dynamic dispatch and restores the descriptor', () => {
  const original = safeObjectGetOwnPropertyDescriptor(Map.prototype, 'get');
  const values = new Map([['key', 'value']]);
  const poison = installPostImportMapGetPoison();
  let failure;
  try {
    values.get('key');
  } catch (error) {
  ...
  assert.equal(failure?.message, 'ATTACKER_MAP_GET');
  assert.equal(poison.traps, 1);
  assert.deepEqual(safeObjectGetOwnPropertyDescriptor(Map.prototype, 'get'), original);
...
function inspectSevenRecordBodyBytes(records) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    rows[index] = {
      ...
      rawSha256: rawSha256(record.bytes),
    };
  }
  return rows;
}

function serializeTask6BArtifactEvidence(bodyInspection, postImportRegExpTestTraps) {
  const records = bodyInspection.map((row) => ({
    id: row.id,
    byteLength: row.byteLength,
    rawSha256: row.rawSha256,
  }));
  return JSON.stringify({
    schema: 'galerina.task6b-export-evidence.v1',
    status: 'PASS',
    expectedHead: GENUINE_COMMIT,
    postImportRegExpTestTraps,
    recordCount: records.length,
    totalBytes: records.reduce((sum, row) => sum + row.byteLength, 0),
    records,
  });
}

function plainChildEnvironment(sourceEnvironment = process.env) {
  const environment = safeObjectCreate(null);
  if (process.platform === 'win32') {
    const descriptor = safeObjectGetOwnPropertyDescriptor(sourceEnvironment, 'SystemRoot');
    const systemRoot = descriptor?.value;
...
test('Task 6B plain child environment rejects inherited execution capabilities', () => {
  const sourceEnvironment = safeObjectCreate(null);
  sourceEnvironment.SystemRoot = 'C:\\Windows';
  sourceEnvironment.PATH = 'attacker-path';
  sourceEnvironment.NODE_OPTIONS = 'attacker-node-options';
  sourceEnvironment.NODE_PATH = 'attacker-node-path';
  sourceEnvironment.GIT_CONFIG_GLOBAL = 'attacker-global';
  sourceEnvironment.GIT_CONFIG_SYSTEM = 'attacker-system';
  sourceEnvironment.ARBITRARY_INHERITED_VALUE = 'attacker-value';
  const environment = plainChildEnvironment(sourceEnvironment);
  const expectedKeys = process.platform === 'win32' ? ['SystemRoot'] : [];
  assert.deepEqual(safeObjectKeys(environment), expectedKeys);
  assert.equal(environment.SystemRoot, process.platform === 'win32' ? 'C:\\Windows' : undefined);
  for (const key of [
    'PATH',
    'NODE_OPTIONS',
    'NODE_PATH',
    ...
    assert.equal(safeObjectHasOwn(environment, key), false, key);
  }
});

test('Task 6B body-free evidence binds each raw body identity', () => {
  const bytes = Buffer.from('{"value":"body-secret-sentinel"}', 'utf8');
  const record = Object.freeze({ id: 'synthetic', bytes });
  const [row] = inspectSevenRecordBodyBytes([record]);
  assert.deepEqual({
    id: row.id,
    byteLength: row.byteLength,
    rawSha256: row.rawSha256,
    ...
    id: 'synthetic',
    byteLength: 32,
    rawSha256: 'b8a085c05e551b691dcd2ecd8626336dd2bf80f877962c88b7b640dc71ac50fa',
  });
});

test('Task 6B body-free evidence diagnostic excludes artifact bodies', () => {
  const diagnostic = serializeTask6BArtifactEvidence([
    {
      id: 'synthetic',
      byteLength: 32,
      rawSha256: 'b8a085c05e551b691dcd2ecd8626336dd2bf80f877962c88b7b640dc71ac50fa',
      bytes: 'MUST_NOT_APPEAR',
    },
  ], 0);
  assert.equal(
    diagnostic,
...
function assertTask6BExporterPoisonSentinel(evidence) {
  ...
  for (let index = 0; index < IDS.length; index += 1) {
    ...
    assert.deepEqual(body, {
      ...
      rawSha256: rawSha256(evidence.records[index].bytes),
    });
...
test('Task 6B exporter result blocks inherited then assimilation', { timeout: 900_000 }, (t) => {
  ...
  t.diagnostic(expectedStdout.trimEnd());
});

test('Task 6B exporter returns the genuine complete seven-body set', { timeout: 900_000 }, async (t) => {
  const head = GENUINE_COMMIT;
  const poison = installPostImportRegExpTestPoison();
  const mapPoison = installPostImportMapGetPoison();
  let records;
  let failure;
  try {
  ...
  t.diagnostic(`postImportMapGetTraps=${mapPoison.traps}`);
  assert.equal(failure, undefined);
  assert.equal(poison.traps, 0);
  assert.equal(mapPoison.traps, 0);
  ...
  const bodyInspection = inspectSevenRecordBodyBytes(records);
  ...
  t.diagnostic(serializeTask6BArtifactEvidence(bodyInspection, poison.traps));
});
