...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    exportName: "normalizeCapability",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits capability normalization through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          ...
          sourceFlowName: "normalizeCapability",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-capability-normalization-slide-"));
    ...
    try {
      ...
      for (const value of VALUES) {
        ...
        assert.equal(verified.value, normalizeCapability(value), JSON.stringify(value));
