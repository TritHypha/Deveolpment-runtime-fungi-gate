...
const BASE_PROGRAM = parseProgram([
  ...
  "contract { effects {} }",
  "{ return Unit }",
].join("\n"), "implicit-return-type-physical-reference.fungi");
...
function publicTypeScriptDecision(returnType) {
  ...
  const result = checkNaming(
    BASE_PROGRAM.ast,
    [{ ...baseFlow, returnType }],
    { fileName: "implicit-return-type-physical-reference.fungi" },
  );
...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    packageIdentity: "@galerina/devtools-naming",
    exportName: "isImplicitReturnType",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits isImplicitReturnType through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        identity: "@galerina/devtools-naming",
        version: "1.0.0-beta.2",
        exports: [{
          name: "isImplicitReturnType",
          sourceFlowName: "isImplicitReturnType",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-implicit-return-slide-"));
    ...
    try {
      ...
      for (const value of VALUES) {
        ...
        assert.equal(receipt.textComparisonWork, Buffer.byteLength(value, "utf8"));
        ...
        assert.equal(verified.value, publicTypeScriptDecision(value), JSON.stringify(value));
      ...
      const exhausted = slide.executeTypedCheckedFungiPackagePublication(
        await prepare(slide, publicationDirectory, context),
        [" void "],
        { steps: 128, textComparisonWork: 5 },
      );
