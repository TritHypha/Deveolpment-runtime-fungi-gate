...
function expected(inputType, operation) {
...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    packageIdentity: "@galerina/cpu-kernels",
    exportName: "requiresLowBitKernel",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits CPU low-bit routing through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        identity: "@galerina/cpu-kernels",
        version: "1.0.0-beta.2",
        exports: [{
          name: "requiresLowBitKernel",
          sourceFlowName: "requiresLowBitKernel",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-cpu-low-bit-slide-"));
    ...
    try {
      ...
      let retainedReceipt;
      const vectors = [
        ...INPUT_TYPES.flatMap((inputType) => OPERATIONS.map((operation) => [inputType, operation])),
        ...HOSTILE,
      ];
      for (const [inputType, operation] of vectors) {
        ...
        assert.equal(verified.value, expected(inputType, operation), `${inputType} x ${operation}`);
