...
const BASE_PROGRAM = parseProgram([
  ...
  "contract { effects {} }",
  "{ return Unit }",
].join("\n"), "generic-type-name-physical-reference.fungi");
...
function publicTypeScriptDecision(typeName) {
  ...
  const result = checkNaming(
    ...
    { fileName: "generic-type-name-physical-reference.fungi" },
  );
...
it(
  "publishes and independently re-admits isGenericTypeName through SLIDE/VOK",
  ...
  async () => {
    ...
    const request = (candidateBytes) => ({
      packages: [{
        ...
        exports: [{
          name: "isGenericTypeName",
          sourceFlowName: "isGenericTypeName",
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-generic-type-slide-"));
    ...
    try {
      ...
      const exhausted = slide.executeTypedCheckedFungiPackagePublication(
        await prepare(slide, publicationDirectory, context),
        [" Any "],
        { steps: 128, textComparisonWork: 4 },
      );
