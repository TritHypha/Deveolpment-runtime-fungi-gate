...
async function prepare(slide, publicationDirectory, context) {
  const prepared = await slide.prepareCheckedFungiPackagePublication({
    ...
    exportName: "isValidRetryStrategy",
    context,
    gates: ALL_ALLOW,
  });
...
it(
  "publishes and independently re-admits isValidRetryStrategy through SLIDE/VOK",
  ...
  async () => {
    ...
    const compiled = slide.compileCheckedFungiPackageSet({
      packages: [{
        ...
        exports: [{
          name: "isValidRetryStrategy",
          sourceFlowName: "isValidRetryStrategy",
          sourceBytes,
        }],
        dependencies: [],
        resources: [],
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-retry-strategy-slide-"));
    ...
    try {
      ...
      for (const [strategy, expected] of VECTORS) {
        ...
        assert.equal(verified.value, expected, JSON.stringify(strategy));
      }
