...
it(
  "publishes and independently re-admits the test marker through SLIDE/VOK",
  ...
  async () => {
    ...
    const parent = await mkdtemp(join(tmpdir(), "galerina-test-marker-slide-"));
    ...
    try {
      ...
      for (const ok of [false, true]) {
        ...
        assert.equal(verified.value, ok ? "✅" : "❌");
