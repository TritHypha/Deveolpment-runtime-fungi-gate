(module
  ...
  (func $validateBackpressurePolicyAtPath (param $p0 i32) (param $p1 i32) (result i32)
    (local $diagnostics i32)
    (local $__fungi_rec_0 i32)
    (local $knownSaturation i32)
    (local $__fungi_rec_1 i32)
    (local $__fungi_rec_2 i32)
    (local $__fungi_rec_3 i32)
    (local.set $diagnostics (call $host___array_create))
    (if (i32.eq (call $isPositiveSafeInteger (f64.load (i32.add (local.get $p0) (i32.const 0)))) (i32.const 0))
      (then
    ...
    (local.set $knownSaturation (i32.const 0))
    (if (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 8))
      (then
        (local.set $knownSaturation (i32.const 1))
      )
      (else
    (if (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 9))
      (then
        (local.set $knownSaturation (i32.const 1))
      )
      (else
    (if (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 10))
      (then
        (local.set $knownSaturation (i32.const 1))
      )
      (else
    ...
    (if (i32.eq (local.get $knownSaturation) (i32.const 0))
      (then
        (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      (local.set $__fungi_rec_1 (global.get $__fungi_heap))
      (global.set $__fungi_heap (i32.add (global.get $__fungi_heap) (i32.const 16)))
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 0)) (i32.const 11)) ;; .code
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 4)) (i32.const 5)) ;; .severity
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 8)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (call $host___str_concat (i32.const 12) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.load (i32.add (local.get $p0) (i32.const 8)))) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.const 13))) ;; .message
    ...
    (if (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 8))
      (then
        (if (i32.or (i32.eq (i32.load (i32.add (local.get $p0) (i32.const 24))) (i32.const 0)) (i32.eq (call $isPositiveSafeInteger (f64.load (i32.add (local.get $p0) (i32.const 16)))) (i32.const 0)))
          (then
            (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      ...
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 12)) (call $host___str_concat (local.get $p1) (i32.const 17))) ;; .path
      ...
      (else
        (if (i32.or (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 10)) (call $host___str_eq (i32.load (i32.add (local.get $p0) (i32.const 8))) (i32.const 9)))
          (then
            (if (i32.eq (i32.load (i32.add (local.get $p0) (i32.const 24))) (i32.const 1))
              (then
                (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      (local.set $__fungi_rec_3 (global.get $__fungi_heap))
      (global.set $__fungi_heap (i32.add (global.get $__fungi_heap) (i32.const 16)))
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 0)) (i32.const 18)) ;; .code
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 4)) (i32.const 5)) ;; .severity
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 8)) (i32.const 19)) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 12)) (call $host___str_concat (local.get $p1) (i32.const 17))) ;; .path
  ...
  (export "validateBackpressurePolicyAtPath" (func $validateBackpressurePolicyAtPath))

  ;; pure flow: validateBackpressurePolicy
  (func $validateBackpressurePolicy (param $p0 i32) (result i32)
    ...
    (call $validateBackpressurePolicyAtPath (local.get $p0) (i32.const 20))
  )
  (export "validateBackpressurePolicy" (func $validateBackpressurePolicy))
