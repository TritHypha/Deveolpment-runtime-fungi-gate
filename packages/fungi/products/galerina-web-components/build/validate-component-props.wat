(module
  ...
  ;; pure flow: validateComponentProps
  (func $validateComponentProps (param $p0 i32) (param $p1 i32) (result i32)
    ...
    (local.set $seenNames (call $host___array_create))
    ...
    (block $while_exit_0
      (loop $while_loop_0
        ...
        (local.set $propOpt (call $host___array_get_option_v2 (local.get $p0) (local.get $index)))
        (local.set $__fungi_match_1 (local.get $propOpt))
        (if (call $host___option_is_none_v2 (local.get $__fungi_match_1))
          ...
          (else
            ...
            (local.set $name (call $host___str_trim (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 0)))))
            (if (i32.eq (call $host___str_length (local.get $name)) (i32.const 0))
              (then
      ...
      (i32.store (i32.add (local.get $__fungi_rec_0) (i32.const 12)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (local.get $p1) (i32.const 4)) (call $host___int_to_str (local.get $index))) (i32.const 5))) ;; .path
      (local.get $__fungi_rec_0)
              ...
              (else
                ...
                (block $while_exit_3
                  (loop $while_loop_3
                    (br_if $while_exit_3 (i32.ge_s (local.get $seenIndex) (call $host___array_length (local.get $seenNames))))
                    ...
                    (local.set $seenOpt (call $host___array_get_option_v2 (local.get $seenNames) (local.get $seenIndex)))
                    ...
                    (if (call $host___option_is_none_v2 (local.get $__fungi_match_4))
                      ...
                      (else
                        ...
                        (if (call $host___str_eq (local.get $__fungi_match_5) (local.get $name))
                          (then
                            (local.set $duplicate (i32.const 1))
                          )
                        )
                      )
                    )
      ...
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 8)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (call $host___str_concat (i32.const 7) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (local.get $name)) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.const 8))) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 12)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (local.get $p1) (i32.const 4)) (call $host___int_to_str (local.get $index))) (i32.const 5))) ;; .path
      (local.get $__fungi_rec_1)
    )))
                  )
                  (else
                    (local.set $seenNames (call $host___array_append (local.get $seenNames) (local.get $name)))
                  )
                )
              )
            )
            (local.set $kindKnown (i32.const 0))
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 9))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 10))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 11))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 12))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 13))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 14))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            (if (call $host___str_eq (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4))) (i32.const 15))
              (then
                (local.set $kindKnown (i32.const 1))
              )
              (else
            ...
            (if (i32.eq (local.get $kindKnown) (i32.const 0))
              (then
                (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      (local.set $__fungi_rec_2 (global.get $__fungi_heap))
      (global.set $__fungi_heap (i32.add (global.get $__fungi_heap) (i32.const 16)))
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 0)) (i32.const 16)) ;; .code
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 4)) (i32.const 2)) ;; .severity
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 8)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (call $host___str_concat (i32.const 17) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.load (i32.add (local.get $__fungi_match_2) (i32.const 4)))) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.const 18))) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 12)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (local.get $p1) (i32.const 4)) (call $host___int_to_str (local.get $index))) (i32.const 19))) ;; .path
  ...
  (export "validateComponentProps" (func $validateComponentProps))
