(module
  ...
  ;; pure flow: validateHtmlParsePlan
  (func $validateHtmlParsePlan (param $p0 i32) (result i32)
    ...
    (if (i32.eq (local.get $knownMode) (i32.const 0))
      (then
        (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      ...
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 8)) (call $host___str_concat (call $host___str_concat (call $host___str_concat (call $host___str_concat (i32.const 11) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.load (i32.add (local.get $p0) (i32.const 4)))) (call $host___int_to_str (call $host___char_from_code (i32.const 34)))) (i32.const 12))) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_1) (i32.const 12)) (i32.const 13)) ;; .path
      (local.get $__fungi_rec_1)
    )))
      )
    )
    (if (i32.eq (call $isPositiveSafeInteger (f64.load (i32.add (local.get $p0) (i32.const 8)))) (i32.const 0))
      (then
        (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      (local.set $__fungi_rec_2 (global.get $__fungi_heap))
      (global.set $__fungi_heap (i32.add (global.get $__fungi_heap) (i32.const 16)))
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 0)) (i32.const 14)) ;; .code
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 4)) (i32.const 5)) ;; .severity
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 8)) (i32.const 15)) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_2) (i32.const 12)) (i32.const 16)) ;; .path
      (local.get $__fungi_rec_2)
    )))
      )
    )
    (if (i32.and (i32.load (i32.add (local.get $p0) (i32.const 24))) (i32.eq (call $isPositiveSafeInteger (f64.load (i32.add (local.get $p0) (i32.const 16)))) (i32.const 0)))
      (then
        (local.set $diagnostics (call $host___array_append (local.get $diagnostics) (block (result i32)
      (local.set $__fungi_rec_3 (global.get $__fungi_heap))
      (global.set $__fungi_heap (i32.add (global.get $__fungi_heap) (i32.const 16)))
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 0)) (i32.const 17)) ;; .code
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 4)) (i32.const 5)) ;; .severity
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 8)) (i32.const 18)) ;; .message
      (i32.store (i32.add (local.get $__fungi_rec_3) (i32.const 12)) (i32.const 19)) ;; .path
  ...
  (export "validateHtmlParsePlan" (func $validateHtmlParsePlan))
