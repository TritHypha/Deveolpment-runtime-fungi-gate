...
before(async () => {
  ...
  parsed = parseProgram(source, "slide-v2c-cbor-validator.fungi", {
  ...
  const result = await run("slideV2CCanonicalReferenceBytes");
  assert.equal(result.audit.result, "ok", JSON.stringify(result.audit));
...
describe("independent SLIDE V2-C canonical-vector admission", () => {
  ...
  it("rejects a mutation at every byte offset", async () => {
    for (let offset = 0; offset < canonicalBytes.length; offset += 1) {
      ...
      assert.equal(field(decision, "failureId").value, "SLIDE-V2C-WIRE-004");
  ...
  it("rejects empty, truncation, and suffixes with terminal identities", async () => {
    for (const [bytes, failureId] of [
      [new Uint8Array(), "SLIDE-V2C-WIRE-002"],
      [canonicalBytes.slice(0, -1), "SLIDE-V2C-WIRE-002"],
      [Uint8Array.from([...canonicalBytes, 0]), "SLIDE-V2C-WIRE-003"],
    ]) {
