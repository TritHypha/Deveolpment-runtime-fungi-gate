...
#   bash gen-certs.sh
#
# Produces (and keeps): ca.crt, TEST-ONLY-server.key, server.crt,
# TEST-ONLY-client-good.key/client-good.crt and
# TEST-ONLY-client-untrusted.key/client-untrusted.crt.
# Intermediate keys/CSRs/serials are deleted.
set -euo pipefail
cd "$(dirname "$0")"
...
openssl req -newkey rsa:2048 -nodes -keyout TEST-ONLY-server.key -out server.csr -subj "/CN=127.0.0.1" >/dev/null 2>&1
...
openssl req -newkey rsa:2048 -nodes -keyout TEST-ONLY-client-good.key -out client-good.csr -subj "/CN=good-client" >/dev/null 2>&1
...
openssl req -newkey rsa:2048 -nodes -keyout TEST-ONLY-client-untrusted.key -out client-untrusted.csr -subj "/CN=rogue-client" >/dev/null 2>&1
