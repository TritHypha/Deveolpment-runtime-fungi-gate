...
> `docs/ROADMAP.md`; dated roadmaps remain
> historical evidence.
...
## SLIDE v2 Readiness Lane

SLIDE v2 is a proposed independent platform target, not a shortcut around the
Core Foundation phases and not an implemented Galerina backend.

Before Galerina emits a production `.slide` bundle or native SLIDE target:
...
3. The Galerina frontend receipt and SLIDE semantic archive must be versioned and
   canonical.
4. `slide.memory.safe-value.v1` negative fixtures must fail closed before native
   execution.
5. Clean and cached action-graph builds must be deterministic and equivalent.
6. A non-Galerina fixture frontend must prove SLIDE is independently usable.
7. The current WebAssembly path remains the differential and separately
   ...
   never an implicit fallback from a failed SLIDE admission.

Implementation for this lane is `.fungi`; `.gate` remains out of scope. See
`docs/architecture/slide-v2-integration-2026-07-29.md` and the canonical status
ledger at
