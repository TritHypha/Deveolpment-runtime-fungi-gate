...
**Status:** **BUILDING (owner-unlocked 2026-07-10: "unlock this now and build as priority").** Steps 1–3 BUILT (`tower-citizen/src/key-rotation.ts`, commit `456c3120`, now hardened to reject MAC-valid impossible enum/tick/lifecycle shapes) with the ratified defaults: M=2 quorum floor · readiness-gated trigger · decision core in tower-citizen · DESTROY-default retire policy (revoke-then-archive selectable, revocation-first enforced). Step 4a BUILT (`sentinel-egress`, commit `9bfdeccc` — epoch MAC-bound into batches, `adoptEpoch` forward-only switch, `verifyChainEpochAware` per-epoch keys, revoked refused, old epochs verify forever). **Step 4b is BUILT 2026-07-30:** Snapshot v2 MAC-binds its non-secret epoch identity; `snapshot-key-provider.ts` verifies the authenticated ring and custody commitment before active/retired-key use; unknown, revoked, substituted, asymmetric, weak, tampered and throwing paths refuse. **Step 5a automatic control is BUILT 2026-07-30:** concrete hybrid transition and candidate-index proof, root-admitted candidate, trigger/readiness/Triple-Lock/switch/canary/fallback/drain/private-retire orchestration, authenticated checkpoint required between every phase, and exact accepted delegation/index anti-rollback identity updated only after canary (Tower 490/490; app-kernel 158/158; State 20/20; Egress 34/34). **Step 5b remains unbuilt:** the production custody/filesystem or HSM adapter that re-signs all admitted package manifests and crash-safely activates the complete candidate artifact set. Disposable-key evidence does not authorize a real ceremony. Decision-1 resolved → both keys, sequenced (attestation first, then the HMAC-ledger asymmetric anchor, §5 step 6).
