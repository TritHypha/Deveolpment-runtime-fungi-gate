...
**Status:** **BUILDING (owner-unlocked 2026-07-10: "unlock this now and build as priority").** Steps 1–3 BUILT (`tower-citizen/src/key-rotation.ts`, commit `456c3120`, 55 tests — ring + all gates + phase machine, zero key material, DI seams fail-closed) with the ratified defaults: M=2 quorum floor · readiness-gated trigger · decision core in tower-citizen · DESTROY-default retire policy (revoke-then-archive selectable, revocation-first enforced). Step 4a BUILT (`sentinel-egress`, commit `9bfdeccc`, 13 tests — epoch MAC-bound into batches, `adoptEpoch` forward-only switch, `verifyChainEpochAware` per-epoch keys, revoked refused, old epochs verify forever). **Step 4b is BUILT 2026-07-30:** Snapshot v2 MAC-binds its non-secret epoch identity; `snapshot-key-provider.ts` verifies the authenticated ring and custody commitment before active/retired-key use; unknown, revoked, substituted, asymmetric, weak, tampered and throwing paths refuse (Sentinel State 20/20; Tower 483/483). **Step 5's production custody executor remains unbuilt; disposable-key evidence does not authorize a real ceremony.** Decision-1 resolved → both keys, sequenced (attestation first, then the HMAC-ledger asymmetric anchor, §5 step 6).
