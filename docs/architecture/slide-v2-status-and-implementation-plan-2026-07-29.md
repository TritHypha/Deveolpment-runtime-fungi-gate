...
| V2-B effect/capability | `SPECIFIED` | Exact database-read/HTTPS requests, lease-only broker, Tower/Tri-Pipe evidence roles, K3 admission, audit-before-success, budgets, failures, mutations, and migration gates | Freeze descriptor; implement request and lease validators before broker dispatch |
