...
## 2026-07-29 SLIDE v2 boundary addendum

The forward native architecture now includes SLIDE v2 as a proposed
**independent execution platform**. Galerina is its first planned frontend, but
SLIDE is not a Core optional provider and must not depend on Galerina at runtime.
The dependency direction is Galerina checks/GIR -> public SLIDE frontend adapter
-> SLIDE semantic archive.

The current WebAssembly/Node-hosted architecture remains the implemented
baseline. SLIDE must not be described as shipped until its independent frontend,
memory-profile, deterministic-build, admission, and final-artifact gates pass.
See `slide-v2-integration-2026-07-29.md`.
