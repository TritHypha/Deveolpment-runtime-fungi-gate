...
- [x] Repair nested audit execution under Node's test runner. Retention probes
  ...
  Tooling is **428 total / 426 pass / 2 intentional skip / 0 fail**.
- [x] Re-run the uninterrupted exhaustive phase-close in the isolated checkout
  after the clean-checkout tooling repairs. The current tree passed all **92/92
  blocking gates** in **1,042.9 seconds**, including the strict canonical
  registry known-answer gate, `build:myco`, `index:myco`, the exact
  compiler-source overwrite audit, **426 passing tooling tests**, and the full
...
- [x] Extend SLIDE's counted-loop reference with the first reusable structured
  body region under Contract 63: one to four Int32 loop-carried values, zero
  to sixteen dense-SSA checked arithmetic instructions, a domain-separated
  body digest, Contract 58 work certification and eleven fail-closed K3 gates.
  Focused evidence is **6/6**, combined counted-loop/K3 evidence is **17/17**,
  and complete SLIDE is **723/723 across 75 suites** with **67 contracts** and
  **68 reference tools**. The region is acyclic and reference-only; general
  control, nested loops, physical `.slide`/VOK binding and authority remain
  open.
- [x] Carry the exact Contract 63 region through canonical binary GIR and the
  context-bound physical `.slide` envelope under Contract 64. Independent
  inspection reconstructs and revalidates the descriptor; preparation creates
  a fresh private Contract 63 plan from source-free bytes. Every single-byte
  object mutation and context drift refuses. Focused evidence is **11/11** and
...
- 🔧 **FIX THE REBUILD PATH — `--force` means the wrong thing** (owner-recorded 2026-07-25, *fix at the right time*, not urgent). `rebuild-fusable-packages.mjs --force` bypasses the **ceremony-SIGNED protection** (`:88`), NOT the freshness skip (`:105`) — the source says so outright at `:108` ("`--force` does NOT bypass the freshness skip above"). Two consequences: (a) on a fresh tree `--force` rebuilds **nothing** while silently arming a CG-7 signed-package bypass, so anyone reaching for it as "regenerate everything" gets the opposite of what they asked for *plus* a risk they didn't ask for; (b) **there is no genuine full-regenerate command at all** — the only way to force a fuse rebuild today is to delete the target `.wasm` and run without the flag. **Proposed shape:** split the flag — `--allow-signed` (today's real meaning, named for what it does) and `--all` / `--rebuild-all` (ignore the freshness skip, never touch signed packages), keep `--force` as a hard error pointing at both so no existing habit silently does the wrong thing. Add a self-test per flag that goes red. Charter already corrected (`.claude/commands/explain.md` §13 "Regenerating the build") so the documented advice is right in the meantime; **I gave the owner the wrong advice about this flag before reading the source — that is why it is on the board.**
