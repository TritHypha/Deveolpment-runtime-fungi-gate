...
**Tech Stack:** Node.js ESM; `node:crypto` Ed25519 classical signature component; `@noble/post-quantum` ML-DSA-65 post-quantum signature component; canonical JSON; SHA-256; `node:test`; existing Galerina stable-handle evidence readers.
...
- [x] Write a failing real-signature test using disposable Ed25519 and ML-DSA-65 keys.
- [x] Run `node --test scripts/tests/beta-release-evidence-envelope.test.mjs` and require module-not-found RED.
- [x] Implement bounded canonical values, role contexts, delegation checks and two-component verification.
- [x] Add mutation cases for each signature half, role, context, key, serial, time, revocation, proxy and accessor boundary.
...
- [x] Write failing literal-fixture tests for a complete durability and repository statement.
- [x] Run the focused test and require module-not-found RED.
- [x] Implement exact in-toto statement subjects and closed Galerina predicates.
- [x] Refuse missing recovery digests, subject mismatch, wrong commands, reordered/missing/nonzero checks and unsafe values.
- [x] Run both receipt and envelope tests and require GREEN.
...
- [x] Convert the fixture to real disposable hybrid signatures and watch the existing verifier fail for the expected schema gap.
- [x] Add negative integration tests for a forged signed predicate, signature downgrade, stale delegation and Boolean-only legacy receipts.
- [x] Implement delegation/public-bundle loading and envelope validation before predicate validation.
- [x] Remove `authenticated: true` handling from durability and repository predicates.
- [x] Preserve K3 `0` only for absent ceremony/external files; cryptographic or semantic failures remain terminal.
- [x] Run platform-smoke and beta admission tests and require GREEN.
...
- `sign-delegation --input <file> --output <file> --root-key-id <id> --operational-ed25519-public <file> --operational-mldsa65-public <file>`.
...
- [x] Write failing CLI tests for malformed/duplicate environment records, wrong key/role, private-value leakage and valid disposable signing.
- [x] Run the focused CLI test and require RED.
- [x] Implement data-only private-environment parsing, role/predicate matching, exclusive output and post-sign self-verification.
- [x] Write the current-state walkthrough with separate “now” and “later offline ceremony” sections.
- [x] Run the CLI tests and path/private-document leak audits.
...
- [x] Record implementation green separately from authority activation.
- [x] Mark the offline evidence delegation and external signed receipts as the only remaining release-admission blockers.
- [x] Correct stale test-count claims encountered in active guidance.
