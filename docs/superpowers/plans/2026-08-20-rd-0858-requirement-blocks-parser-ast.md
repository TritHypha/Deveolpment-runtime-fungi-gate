...
Expected: the governed wrapper publishes its complete graph output set and
provenance sidecar; generation and integrity exit zero with nonzero node/edge
counts.
...
Generated provenance names the exact source/tool commit consumed immediately
before this evidence-only commit. It cannot name the commit that contains
itself without creating an infinite commit cycle. After the evidence commit,
run the generators' non-mutating `--check` modes; those checks compare all
stable provenance fields and generated bytes while deliberately excluding only
the timestamp and self-referential Git field. A failed check is `HOLD`.
