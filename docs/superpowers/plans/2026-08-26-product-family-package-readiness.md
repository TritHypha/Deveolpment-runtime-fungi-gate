...
assert.throws(() => generateProductProfiles(sourceWithPolicyDigest, readExactPolicy), /SOURCE_SCHEMA_REFUSED/);
assert.throws(() => generateProductProfiles(sourceWithWrongSchema, readExactPolicy), /SOURCE_SCHEMA_REFUSED/);
assert.throws(() => generateProductProfiles(sourceWithUnknownField, readExactPolicy), /SOURCE_SCHEMA_REFUSED/);
```

The last three controls use exact source bytes and prove that a generated-only
field, a substituted source-schema identity and any unknown source field refuse
before transformation. A controlled mutation that removes the pre-transform
source validation must make all three controls fail.
...
node --test packages-galerina/galerina-core-compiler/tests/product-profile.test.mjs
```

Expected: FAIL because `loadProductRegistry` and `resolveProductProfile` are not exported.
...
the source-schema refusal tests as red-capability controls: deleting or moving
the source validation below transformation must fail the focused test.
