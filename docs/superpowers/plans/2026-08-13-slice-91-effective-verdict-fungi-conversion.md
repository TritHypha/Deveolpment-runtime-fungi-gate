# Slice 91 `effectiveVerdict` Fungi Conversion Implementation Plan
...
**Goal:** Prove or fail closed on one typed, reference-only Fungi replacement for Tower-Citizen's `effectiveVerdict` Kleene minimum.

**Architecture:** A package-owned pure flow accepts two typed Verdicts and implements the exact nine-row K3 minimum with nested exhaustive `check`. A package differential test binds it to the TypeScript oracle; a separate physical proof compiles, publishes and independently re-admits one `.slide` through VOK. TypeScript remains active.
...
- Pin Galerina to the live Slice 91 branch head and SLIDE to `ed326eaa14f1a899841cbac8da353d400970367e` before physical proof.
- Preserve all nine literal `min(ideal, reading)` rows as typed Verdicts.
...
- Do not use an Int/Bool bridge, host projection, widened SLIDE profile or new authority.
...
**Files:**
- Create: `packages-galerina/galerina-tower-citizen/tests/effective-verdict-fungi-conversion.test.mjs`
- Create after RED: `packages-galerina/galerina-tower-citizen/src/self-hosted/effective-verdict.fungi`
- Modify after RED: `packages-galerina/galerina-tower-citizen/package.json`

**Interfaces:**
- Consumes: exported TypeScript `effectiveVerdict`, `Verdict`, a hand-derived nine-row K3 minimum table, Galerina `parseProgram`, `checkEffects` and `executeFlow`.
- Produces: `pure flow effectiveVerdict(ideal: Verdict, reading: Verdict) -> Verdict` and one exact `packageGraph.loadedAssets` entry.

- [ ] **Step 1: Write the failing ownership and parity test**

Create nine literal triples, require the missing asset and loaded-asset entry, bind the exact exported `return vAnd(ideal, reading)` source chain, reject forbidden syntax and compare TypeScript plus interpreted Fungi values for every row.

- [ ] **Step 2: Run RED**

Build Tower-Citizen, then run only the new test. It must fail because the asset is absent, not because the fixture or imports are wrong.

- [ ] **Step 3: Add the minimal typed candidate and declaration**

```fungi
@version 1
pure flow effectiveVerdict(ideal: Verdict, reading: Verdict) -> Verdict {
  check(ideal) {
...
Add `src/self-hosted/effective-verdict.fungi` to `packageGraph.loadedAssets` without changing runtime exports or entrypoints.

- [ ] **Step 4: Run GREEN and strict checks**

Run the focused test, the existing substrate-model safety theorem and strict candidate check. All nine rows and all existing degrade-only assertions must pass with zero candidate diagnostics.

- [ ] **Step 5: Commit the candidate wave**

Stage only the test, candidate and package declaration. Commit `add slice 91 effective verdict candidate`.

### Task 2: Prove the two-Verdict physical SLIDE/VOK boundary

**Files:**
- Create: `scripts/tests/tower-citizen-effective-verdict-fungi-slide.integration.test.mjs`
...
Record the exact diagnostic, remove candidate and declaration, retain stable negative evidence and do not substitute integers or widen SLIDE.

- [ ] **Step 5: Commit the physical proof**

Stage only the integration test. Commit `prove slice 91 through physical slide`.

### Task 3: Close the governed slice

**Files:**
- Create: `docs/reports/slice-91-effective-verdict-fungi-conversion-2026-08-13.md`
...
Run the slice-receipt audit and self-test, canonical counts, path/private leak gates, semantic graph check, percent freshness, status and roadmap checks. Excluded aggregate lanes stay excluded.

- [ ] **Step 5: Commit, regenerate registered owners and refresh indexes**

Commit authored documents first. Regenerate registered roadmap/subway/project-graph owners against that commit, verify them and commit only their outputs. At final HEAD, moderately refresh codebase-memory and require exact expected nodes, exact `indexed_head_sha`, `stale: false` and a queryable Slice 91 symbol. Re-index Myco and report its bounded counts.
