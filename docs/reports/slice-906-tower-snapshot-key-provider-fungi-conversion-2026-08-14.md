# Slice 906 src/snapshot-key-provider.ts#TowerSnapshotKeyProvider Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/snapshot-key-provider.ts#TowerSnapshotKeyProvider`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public provider declaration until bindings preserve exact active/resolve callback behavior and nullable handle results.

Minimum vectors: active/resolve; epoch/key selectors; null and handle; callback throw/reentry; external declaration compatibility.
