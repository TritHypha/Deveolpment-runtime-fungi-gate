# Slice 913 src/snapshot-key-provider.ts#createSnapshotKeyProvider.resolve Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/snapshot-key-provider.ts#createSnapshotKeyProvider.resolve`.

`BLOCKED` (BLOCKED_BY_RESOLVE_INPUT_NUMBER_STRING_SELECTOR_ACTIVE_RING_CUSTODY_SECRET_ALIAS_NULL_AND_METHOD_ABI). Exact exit: admit exact number/String ingress, the complete resolveKey chain and epochForVerification selector with an approved Option/typed-failure mapping and secret-byte lease/copy ABI.

Minimum vectors: epochId NaN/infinity/-0/fraction/unsafe/0/1/max; keyId wrong class/empty/case/mismatch/getter/proxy; absent/retired/revoked/asymmetric/duplicate epochs; callback/commit failures; returned alias mutation.
