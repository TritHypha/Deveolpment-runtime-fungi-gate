# Slice 691 policyCacheKey Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/gate-cache.ts#policyCacheKey`.

`BLOCKED` (BLOCKED_BY_SAME_SNAPSHOT_CANONICAL_POLICY_SHA256_AND_CRYPTO_ABI): Hash the exact admitted compilation snapshot with pinned SHA-256/domain identity, typed failures and physical crypto receipt.

Minimum vectors: All canonical vectors; exact UTF-8/64 hex; one-byte changes; algorithm failure/substitution.
