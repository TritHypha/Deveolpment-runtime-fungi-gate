# Slice 926 src/substrate-erasure.ts#canonicalSubstrate Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#canonicalSubstrate`.

`BLOCKED` (BLOCKED_BY_ACTIVE_MANIFEST_OBJECT_ENTRIES_UNDEFINED_FILTER_AMBIENT_LOCALECOMPARE_OBJECT_REBUILD_JSON_AND_CANONICAL_STRING_ABI). Exact exit: capture one exact own-data manifest, refuse surplus/toJSON/accessor/proxy states, and use an owner-versioned locale-independent total byte/code-unit order or completely pin the source locale/ICU/runtime oracle; bind omission, JSON escaping and exact signer/verifier bytes.

Minimum vectors: field permutations; missing/undefined/null; surplus/numeric/toJSON/__proto__/constructor; getters/proxies/mutation; Unicode/lone surrogates/normalization/cross-locale/ICU; circular/BigInt/undefined/throw.
