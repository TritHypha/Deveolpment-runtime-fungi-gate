# Slice 562 resolveSensitivity Fungi conversion adjudication

Scope: `packages-galerina/galerina-tools-myco/src/query/search.ts#resolveSensitivity`.

`BLOCKED` (BLOCKED_SMART_CASE_UNICODE_LOWERCASE_AND_ERASED_OPTION_ABI): The exact named boundary remains unproved. Exit requires exact admitted inputs, state/effect/failure and cleanup order, finite work, GIR, physical .slide, independent re-admission, VOK, consumer switch and hostile differential proof.

Minimum vectors: true/false/smart; ASCII/Unicode titlecase, uncased, Turkish I, decomposed and astral case; surplus/coercive modes.
