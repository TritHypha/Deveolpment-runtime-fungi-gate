# Slice 228 JsonLineSink.write Fungi conversion adjudication

## Outcome

`JsonLineSink.write` is `BLOCKED_BY_JSON_LINE_CALLBACK_EGRESS_ABI`. Exact host JSON behavior, synchronous callback egress and writer failures are one unproved effect boundary. No placeholder Fungi asset is created.

Required exit: Bind canonical bytes to an admitted serializer and separately govern the writer effect and its failure receipt.
