# Slice 751 src/key-rotation.ts#isNonNegativeSafeInteger Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#isNonNegativeSafeInteger`.

`BLOCKED` (BLOCKED_BY_JAVASCRIPT_SAFE_INTEGER_BINARY64_TO_PHYSICAL_I32_DOMAIN). Exact exit: admit the full ECMAScript unknown/binary64 Number.isSafeInteger domain through 2^53-1 or owner-narrow and re-prove every caller; an i32 profile alone is insufficient.

Minimum vectors: undefined, null, String, NaN, infinities, fraction, -1, -0/0, 2^31-1, 2^31, 2^53-1 and 2^53.
