# Slice 766 src/key-rotation.ts#lockAVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#lockAVerdict`.

`BLOCKED` (BLOCKED_BY_CRYPTO_RING_SNAPSHOT_TYPED_ARRAY_CALLBACK_AND_COLLECTION_ABI). Exact exit: bind one immutable ring/key snapshot to independently admitted MAC and round-trip verifier receipts, preserving epoch traversal, case mapping, callbacks and physical record ABI.

Minimum vectors: tampered/weak/detached key, candidate statuses, duplicate commits/IDs/files, missing active, callback nontrue/throw and split getters.
