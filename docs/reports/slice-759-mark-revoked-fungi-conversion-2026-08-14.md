# Slice 759 src/key-rotation.ts#markRevoked Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#markRevoked`.

`BLOCKED` (BLOCKED_BY_RING_CRYPTO_REVOCATION_ALIAS_THROW_AND_NULL_ABI). Exact exit: use one verified immutable ring/key/id/tick snapshot, preserve target/status/tick checks and typed errors, define identity policy and MAC the same derived revocation state.

Minimum vectors: unknown/duplicate ID, active/staged/revoked target, retiredTick variants, invalid tick, getter mutation, unchanged alias mutation and crypto failure.
