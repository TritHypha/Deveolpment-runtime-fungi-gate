# Slice 643 admitRowVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/data-plane-border.ts#admitRowVerdict`.

`BLOCKED` (BLOCKED_BY_PROVEN_SCOPE_POLICY_SET_AND_K3_ADMISSION_ABI): Require provenance-bound user scope/policy receipts, one inert snapshot and exact K3 physical admission.

Minimum vectors: all lane combinations; own/foreign/shared rows; fake/mutable Set; forged policy; inherited registry; Unicode keys.
