# Slice 755 src/key-rotation.ts#epochForVerification Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#epochForVerification`.

`BLOCKED` (BLOCKED_BY_LIVE_ARRAY_FIND_VERIFICATION_AUTHORITY_ALIAS_NULL_AND_NUMBER_ABI). Exact exit: admit exact live find, strict-number and alias behavior, or use a verified one-snapshot ring and Option of immutable verification capability bound to MAC and revocation state.

Minimum vectors: unknown/future/duplicate IDs, NaN/Infinity/fraction/-0, every status, holes/getters/proxies and returned alias mutation.
