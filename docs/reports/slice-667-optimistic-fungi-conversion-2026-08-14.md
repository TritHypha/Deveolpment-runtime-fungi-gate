# Slice 667 optimistic Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#optimistic`.

`BLOCKED` (BLOCKED_BY_GENERIC_ACTIVE_VALUE_UTF16_PROVENANCE_RECORD_IDENTITY_ABI): Bind a closed payload ABI, exact reason semantics and a copy/freeze or retained-identity ruling before physical record admission.

Minimum vectors: Primitive/active payload; alias mutation; default/control/Unicode reason; coercive/throwing reason; output mutation.
