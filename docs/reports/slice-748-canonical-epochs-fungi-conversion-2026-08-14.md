# Slice 748 src/key-rotation.ts#canonicalEpochs Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#canonicalEpochs`.

`BLOCKED` (BLOCKED_BY_ACTIVE_EPOCH_ARRAY_JSON_STRINGIFY_AND_CANONICAL_WIRE_ABI). Exact exit: admit one bounded immutable own-data KeyEpoch array and exact ECMAScript JSON bytes, or version and atomically migrate an injective canonical encoding in both MAC producer and verifier; bind number, Option, String and Array through physical SLIDE/VOK.

Minimum vectors: empty, sparse, undefined, null, -0/0, NaN/Infinity, BigInt throw, getter/proxy mutation and order permutation.
