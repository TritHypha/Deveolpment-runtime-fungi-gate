# Slice 693 GateCache class boundary Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/gate-cache.ts#GateCache`.

`BLOCKED` (BLOCKED_BY_MUTABLE_MAP_COUNTER_AND_COMPILED_POLICY_ALIAS_IDENTITY_ABI): Admit exact active Map/counter state or redesign as bounded immutable capability cache with non-aliasing policy receipts.

Minimum vectors: Independent instances; Map order/SameValueZero; repeated identity; counters; unbounded entries; reentrancy.
