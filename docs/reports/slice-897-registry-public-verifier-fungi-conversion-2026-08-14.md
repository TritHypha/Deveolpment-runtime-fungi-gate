# Slice 897 src/registry-public-verifier.ts#RegistryPublicVerifier Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/registry-public-verifier.ts#RegistryPublicVerifier`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public callback type until governed bindings preserve message-view identity, signature/key-ID Strings, invocation/failure behavior, callback identity/reentrancy and the closed true/false/no-key result.

Minimum vectors: matched/mismatched key; canonical/malformed/oversized signature; offset/detached/resizable/SAB/mutated message; all three outcomes; runtime throw/foreign result; callback identity/reentrancy.
