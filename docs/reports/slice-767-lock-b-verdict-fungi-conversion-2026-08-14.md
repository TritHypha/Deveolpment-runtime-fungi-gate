# Slice 767 src/key-rotation.ts#lockBVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#lockBVerdict`.

`BLOCKED` (BLOCKED_BY_CRYPTO_RING_TRANSITION_CALLBACK_ORDER_AND_PHYSICAL_RECORD_ABI). Exact exit: bind one immutable ring/transition/head snapshot to admitted old/new MAC receipts and preserve eager B1 then old then new callback order and failure conservation.

Minimum vectors: tampered ring, empty/mismatched head, half signatures, wrong epochs, callback absence/nontrue/throw/order and mutation after verify.
