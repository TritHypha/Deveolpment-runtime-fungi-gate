# Slice 788 src/key-rotation.ts#confirmDrain Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#confirmDrain`.

`BLOCKED` (BLOCKED_BY_DRAIN_EVIDENCE_QUORUM_CALLBACK_AND_PHASE_AUTHORITY_ABI). Exact exit: capture immutable drain/votes/quorum/context, preserve or owner-change drain work before phase refusal, and prove diagnostic and phase/log ordering physically.

Minimum vectors: wrong phase, count extrema, duplicate/proxy votes, m boundaries, diagnostic throw and same-reference refusal.
