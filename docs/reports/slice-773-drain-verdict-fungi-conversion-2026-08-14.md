# Slice 773 src/key-rotation.ts#drainVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#drainVerdict`.

`BLOCKED` (BLOCKED_BY_LIVE_DRAIN_SNAPSHOT_SIGNER_QUORUM_ARRAY_AND_GATE_ABI). Exact exit: capture counts and votes in one admitted snapshot, preserve source number domain and physically prove quorum and GateResult evidence.

Minimum vectors: negative/fraction/NaN/infinity/-0/unsafe counts, zero permutations, m edges, equivocation and proxy/vote mutation.
