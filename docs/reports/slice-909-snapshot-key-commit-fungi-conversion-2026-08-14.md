# Slice 909 src/snapshot-key-provider.ts#snapshotKeyCommit Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/snapshot-key-provider.ts#snapshotKeyCommit`.

`BLOCKED` (BLOCKED_BY_LIVE_SECRET_BYTES_NODE_SHA256_DOMAIN_PREIMAGE_ERROR_IDENTITY_AND_HEX_ABI). Exact exit: hold one immutable secret-byte snapshot across strength check and SHA-256 over the exact context-plus-NUL preimage, with custody, provider receipt, lowercase-hex ABI and exact TypeError/failure route.

Minimum vectors: weak boundaries; repeated low-entropy accepted; one-byte changes; context/NUL separation; offset/alias/detach/SAB mutation; crypto failure; exact hex/error.
