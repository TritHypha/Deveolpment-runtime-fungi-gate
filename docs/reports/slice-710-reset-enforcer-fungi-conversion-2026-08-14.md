# Slice 710 governance-enforcer.ts#GovernanceEnforcer.reset Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer.reset`.

`BLOCKED` (BLOCKED_BY_ORDERED_AUTHORITY_ERASURE_AND_SHARED_STATE_LIFECYCLE_ABI). Exact exit: use an affine execution context with exactly-once ordered authority erasure before success or failure release, typed repeated/reset-before-use outcomes and an independent cleanup receipt.

Minimum vectors: Partial states repeated reset and interleaved consumer.
