# Slice 554 SearchOptions Fungi conversion adjudication

Scope: `packages-galerina/galerina-tools-myco/src/query/search.ts#SearchOptions`.

`NO_RUNTIME_BEHAVIOR` (ERASED_INTERFACE_RETAIN_DECLARATION): This declaration is erased at runtime. Retain or version its exact TypeScript developer contract; no placeholder Fungi execution authority or source retirement follows.

Minimum vectors cover missing, surplus, inherited, accessor, proxy and wrong-class fields plus every closed literal/tag and mutable-array alias.
