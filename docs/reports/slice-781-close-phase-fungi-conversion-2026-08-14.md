# Slice 781 src/key-rotation.ts#closePhase Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#closePhase`.

`BLOCKED` (BLOCKED_BY_HIGHER_ORDER_APPLY_DIAGNOSTIC_CALLBACK_IDENTITY_ORDER_AND_PROCESS_STATE_ABI). Exact exit: admit a closed phase/result/apply state machine preserving phase diagnostic order, authorized-only apply, callback throws/partial effects, same-reference refusal and exact log append order.

Minimum vectors: every phase mismatch, forged verdict, diagnostic throw/mutate, apply not-called/throw/malformed, proxy process/log and same-reference refusal.
