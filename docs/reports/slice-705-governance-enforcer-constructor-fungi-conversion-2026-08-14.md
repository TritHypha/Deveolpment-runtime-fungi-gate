# Slice 705 governance-enforcer.ts#GovernanceEnforcer.constructor Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer.constructor`.

`BLOCKED` (BLOCKED_BY_ACTIVE_POLICY_ALIAS_ITERATION_ERROR_IDENTITY_AND_EXACT_DECODER). Exact exit: capture one ordinary-own-data policy snapshot, decode closed transition and requirement domains, map iteration/coercion failures to typed load refusal, retain no caller alias, and prove construction precedence and physical parity.

Minimum vectors: Sparse proxy arrays iterator failure and post-construction mutation.
