# Slice 774 src/key-rotation.ts#RetirePolicyMode Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#RetirePolicyMode`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the closed public mode union until governed bindings replace the declaration for all consumers.

Minimum vectors: every exact mode plus case/surplus/wrong-class values.
