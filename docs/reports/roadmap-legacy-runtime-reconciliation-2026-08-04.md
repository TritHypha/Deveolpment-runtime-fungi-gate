...
repeat or relabel that evidence as production SLIDE.
...
| Former `subprojects/dss-host` | Superseded | Keep no production sidecar; its useful evidence moved into the flat development-only oracle | A future Rust/Wasmtime production TCB |
...
| Historical `#102` embedder | Retired architecture | The old production Wasmtime embedder is not rebuilt | A living roadmap task |
...
| Self-hosting stages 3–6 label | Ambiguous and stale | Rename the actual remainder to the SLIDE bootstrap fixpoint: executable source-to-SLIDE self-compile, exact fixpoint, crypto FFI and bounded host path | Repeating the completed stage-flip/Wasm programme |
...
- Completed evidence is reused as a differential oracle; it is not rebuilt to
  make a renamed milestone look new.
