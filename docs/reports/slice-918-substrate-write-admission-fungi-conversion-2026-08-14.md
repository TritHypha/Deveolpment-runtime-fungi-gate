# Slice 918 src/substrate-erasure.ts#SubstrateWriteAdmission Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#SubstrateWriteAdmission`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the BoundaryDecision/admitted/model/reason declaration until an exact governed binding and every consumer are proved.

Minimum vectors: all Verdict/authorized relations; both models; admitted consistency; reason String domain; missing/surplus fields.
