# Slice 915 src/substrate-erasure.ts#EraseModel Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#EraseModel`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public declaration or replace it only through a governed schema generator preserving the exact two-literal union and every consumer.

Minimum vectors: overwrite; crypto-only; case/spelling/surplus; external declaration compatibility.
