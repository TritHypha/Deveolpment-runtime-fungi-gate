# Slice 785 src/key-rotation.ts#switchEpoch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#switchEpoch`.

`BLOCKED` (BLOCKED_BY_LIVE_RING_TYPED_ARRAY_EXCEPTION_CALLBACK_AND_SWITCH_ATOMICITY_ABI). Exact exit: bind ring/key/tick and Error mapping to one snapshot; preserve source precedence in which `switchActive` and its caught failure execute before `closePhase` checks `locked`, unless an owner-approved versioned change migrates that order; make the old-retired/new-active switch atomic and prove it physically.

Minimum vectors: wrong phase combined with switch Error, non-Error, provider and key-view failure; active/staged multiplicity; invalid/backward ticks; callback mutation; and refusal identity.
