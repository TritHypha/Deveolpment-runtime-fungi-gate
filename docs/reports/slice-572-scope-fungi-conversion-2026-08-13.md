# Slice 572 rank Fungi conversion adjudication

Scope: `packages-galerina/galerina-tools-myco/src/query/search.ts#rank`.

`BLOCKED` (MYCO-SEARCH-RANK-MAP-SORT-001): The exact named boundary remains unproved. Exit requires exact admitted inputs, state/effect/failure and cleanup order, finite work, GIR, physical .slide, independent re-admission, VOK, consumer switch and hostile differential proof.

Minimum vectors: Every rank tie; astral paths; stable equal records; nonfinite coordinates, sparse/getter/proxy data and alias mutation.
