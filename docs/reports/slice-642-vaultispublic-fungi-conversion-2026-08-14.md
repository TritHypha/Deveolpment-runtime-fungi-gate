# Slice 642 vaultIsPublic Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/data-plane-border.ts#vaultIsPublic`.

`BLOCKED` (BLOCKED_BY_ATTESTED_CLOSED_VAULT_REGISTRY_ABI): Require an authenticated prototype-neutral own-data registry snapshot and exact token identity.

Minimum vectors: own/absent/inherited public entry; getter/proxy throw/change; token case/NUL; registry mutation.
