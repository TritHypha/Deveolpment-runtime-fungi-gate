# Slice 786 src/key-rotation.ts#confirmTripleVerify Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#confirmTripleVerify`.

`BLOCKED` (BLOCKED_BY_VERIFY_SEAM_CALLBACK_EVIDENCE_AND_PHASE_AUTHORITY_ABI). Exact exit: admit exact evidence/context and probe effects, preserving or owner-changing probe-before-phase order, then prove K3 collapse, diagnostic and state update physically.

Minimum vectors: wrong phase probe effects, allow/deny/throw probes, hash mismatch, numeric extrema, proxy mutation and callback order.
