# Slice 760 src/key-rotation.ts#RotationCtx Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#RotationCtx`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public declaration until a governed binding generator preserves all six optional synchronous callback signatures and every external consumer.

Minimum vectors: each callback absent/present, exact Boolean result typing and external declaration compatibility.
