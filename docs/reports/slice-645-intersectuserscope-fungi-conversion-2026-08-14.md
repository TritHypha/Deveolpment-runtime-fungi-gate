# Slice 645 intersectUserScope Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/data-plane-border.ts#intersectUserScope`.

`BLOCKED` (BLOCKED_BY_ORDERED_FILTER_CALLBACK_AND_BOUNDED_ITERATION_ABI): Require bounded ordered iteration over inert rows plus an admitted narrowing callback/effect ABI.

Minimum vectors: empty/dense/sparse/duplicate arrays; mutation; denied row skips callback; callback true/false/throw/effects; overridden filter.
