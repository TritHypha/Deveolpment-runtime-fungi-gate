# Slice 703 governance-enforcer.ts#GovernanceEnforcer Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer`.

`BLOCKED` (BLOCKED_BY_ACTIVE_TRANSITION_POLICY_CRYPTO_AND_MUTABLE_AUTHORITY_STATE_ABI). Exact exit: replace caller-mintable state with an immutable decoded policy, provenance-bound validation and real signature receipts, branded transitions, an affine execution lifecycle and typed erasure/failure outcomes, then admit the complete state/effect profile.

Minimum vectors: Cross-consumer state bleed and sign validate reset interleavings.
