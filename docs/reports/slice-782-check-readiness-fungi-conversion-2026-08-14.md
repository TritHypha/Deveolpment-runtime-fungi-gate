# Slice 782 src/key-rotation.ts#checkReadiness Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#checkReadiness`.

`BLOCKED` (BLOCKED_BY_PHASE_AUTHORITY_CALLBACK_RECORD_ALIAS_AND_K3_ADMISSION_ABI). Exact exit: admit immutable process/evidence/context and callback effects, explicitly preserve or owner-change readiness probing before phase refusal, then prove the phase transition through GIR/SLIDE/re-admission/VOK.

Minimum vectors: wrong phase, count boundaries, probe allow/deny/throw, accessors/mutation, diagnostic throw and log/same-reference order.
