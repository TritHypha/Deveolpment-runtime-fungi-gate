# Slice 917 src/substrate-erasure.ts#WritePayload Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#WritePayload`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the exact two-Boolean public declaration until governed bindings and all consumers replace it.

Minimum vectors: all Boolean pairs; truthy non-Booleans; missing/surplus/inherited/accessor/proxy; readonly erasure.
