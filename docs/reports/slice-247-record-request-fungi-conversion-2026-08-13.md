# Slice 247 recordRequest Fungi conversion adjudication

## Outcome

`recordRequest` is `BLOCKED_BY_CLOCK_CONTEXT_METRICS_RECORD_TRANSACTION_ABI`. It reads an active context, computes binary64 duration and performs an ordered shared collector mutation whose failure is suppressed. No placeholder Fungi asset is created.

Required exit: Admit the exact context record, clock arithmetic and serialized metrics mutation with typed failure containment.
