# Slice 784 src/key-rotation.ts#commitTripleLock Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#commitTripleLock`.

`BLOCKED` (BLOCKED_BY_TRIPLE_LOCK_RING_QUORUM_CALLBACK_AND_PHASE_AUTHORITY_ABI). Exact exit: capture immutable ring/transition/votes/context, preserve or owner-change seam execution before phase refusal, admit callback effects and quorum, and physically prove the state transition.

Minimum vectors: wrong phase and seam calls, absent/multiple staged, all K3 combinations, key views, duplicate votes, m extrema and callback throws.
