# Slice 659 combine Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#combine`.

`BLOCKED` (BLOCKED_BY_EFFECTFUL_CALLBACK_AND_SPLIT_TWO_INPUT_SNAPSHOT): Capture both inputs once, admit callback effects/failure, preserve exact K3 fold and ordered immutable provenance, then prove physical parity.

Minimum vectors: Full 3x3 trust table; callback throw/mutate/re-enter; getters/proxies; invalid trust; alias/order.
