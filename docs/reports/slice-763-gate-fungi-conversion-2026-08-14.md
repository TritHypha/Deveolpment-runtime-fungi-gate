# Slice 763 src/key-rotation.ts#gate Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#gate`.

`BLOCKED` (BLOCKED_BY_MUTABLE_CLAUSE_ARRAY_K3_FOLD_REASON_STRING_AND_RECORD_ABI). Exact exit: capture one bounded dense tuple array, mint every Verdict, preserve empty/single/multi allOf and reason order/bytes, close sparse/proxy behavior and return immutable evidence physically bound to the verdict.

Minimum vectors: empty, each singleton, pair/triple K3 orders, holes/non-Verdict, proxy mutation, control-text reasons and returned-array mutation.
