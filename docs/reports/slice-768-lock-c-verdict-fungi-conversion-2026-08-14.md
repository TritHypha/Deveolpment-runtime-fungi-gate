# Slice 768 src/key-rotation.ts#lockCVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#lockCVerdict`.

`BLOCKED` (BLOCKED_BY_SIGNER_VOTE_ARRAY_QUORUM_IDENTITY_AND_PHYSICAL_VERDICT_ABI). Exact exit: admit closed SignerVote records and arrays with distinct-signer/no-equivocation semantics, exact m-number domain and physical Verdict/quorum proof.

Minimum vectors: m NaN/infinity/fraction/<2/unsafe, empty/short/duplicate/equivocating votes, holes/getters/proxies and live mutation.
