# Slice 765 src/key-rotation.ts#readinessVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#readinessVerdict`.

`BLOCKED` (BLOCKED_BY_CALLBACK_SEAM_CAPTURE_ONCE_NUMERIC_RECORD_AND_PHYSICAL_GATE_ABI). Exact exit: admit exact own-data readiness evidence captured once, preserve source number domain and callback absence/nontrue/throw/eager order, and prove GateResult/reasons physically.

Minimum vectors: negative/fraction/NaN/infinity/-0/unsafe counts, equal/over queue, split getters and callback mutate/throw/order.
