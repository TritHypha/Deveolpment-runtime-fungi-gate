# Slice 223 LogSink Fungi conversion adjudication

## Outcome

`LogSink` is `BLOCKED_BY_ACTIVE_LOG_SINK_CAPABILITY_ABI`. The erased protocol still describes an arbitrary synchronous output capability whose identity, ordering, effects, failure and revocation have no admitted active boundary. No placeholder Fungi asset is created.

Required exit: Define an affine sink lease with explicit effects, failure receipts, revocation and ordering before reopening.
