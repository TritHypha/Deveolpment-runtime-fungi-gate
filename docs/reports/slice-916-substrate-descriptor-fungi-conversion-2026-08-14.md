# Slice 916 src/substrate-erasure.ts#SubstrateDescriptor Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#SubstrateDescriptor`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public declaration until bindings preserve id, claimed model, optional attested presence and all consumers without treating the type as runtime provenance.

Minimum vectors: missing versus own undefined; both models; attested true/false/1; missing/surplus/inherited/accessor/proxy; readonly erasure.
