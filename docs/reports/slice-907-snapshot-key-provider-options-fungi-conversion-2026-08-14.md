# Slice 907 src/snapshot-key-provider.ts#SnapshotKeyProviderOptions Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/snapshot-key-provider.ts#SnapshotKeyProviderOptions`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public options declaration until bindings preserve callback identities, custody behavior, ringMacKey Uint8Array and all consumers.

Minimum vectors: required callbacks; missing/surplus/inherited/accessor/proxy; live key aliases; readonly erasure.
