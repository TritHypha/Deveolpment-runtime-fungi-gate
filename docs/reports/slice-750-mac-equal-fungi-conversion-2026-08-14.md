# Slice 750 src/key-rotation.ts#macEqual Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#macEqual`.

`BLOCKED` (BLOCKED_BY_NODE_BUFFER_UTF8_TIMING_SAFE_EQUAL_AND_ERROR_ABI). Exact exit: admit exact Node UTF-8 and equal-length timing-safe comparison with typed Buffer/length/provider failures, or migrate all callers to strict decoded 32-byte digests; prove the timing-safe physical primitive.

Minimum vectors: equal, unequal length, first/last mismatch, upper/lower/nonhex, non-ASCII/lone surrogate and provider throw.
