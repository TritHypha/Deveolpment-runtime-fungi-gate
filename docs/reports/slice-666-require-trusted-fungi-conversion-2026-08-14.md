# Slice 666 requireTrusted Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#requireTrusted`.

`BLOCKED` (BLOCKED_BY_CALLER_MINTABLE_PROVEN_RECORD_ONE_SNAPSHOT_AUDIT_RELEASE_ABI): Require an unforgeable receipt, one inert capture, Option value ABI, exact audit ordering/failure and physical release admission.

Minimum vectors: Full trust table; forged/mutated +1; getters/proxy split reads; provenance alias; callback throw/re-entry.
