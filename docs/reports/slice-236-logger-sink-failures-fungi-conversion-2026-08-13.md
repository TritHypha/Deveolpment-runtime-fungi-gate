# Slice 236 Logger.sinkFailures Fungi conversion adjudication

## Outcome

`Logger.sinkFailures` is `BLOCKED_BY_MUTABLE_LOG_FAILURE_COUNTER_OBSERVATION_ABI`. It reads live per-instance mutable state whose provenance and ordering cannot be projected into a scalar. No placeholder Fungi asset is created.

Required exit: Admit the logger identity and failure ledger; also separate sink failures from construction/redaction failures.
