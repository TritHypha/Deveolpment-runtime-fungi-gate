# Slice 775 src/key-rotation.ts#RetirePolicy Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#RetirePolicy`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public record declaration until governed bindings preserve mode and Boolean surface without treating typing as authority.

Minimum vectors: each mode/Boolean combination, missing/surplus/accessor fields and declaration compatibility.
