# Slice 761 src/key-rotation.ts#seamVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#seamVerdict`.

`BLOCKED` (BLOCKED_BY_EFFECTFUL_HOST_CALLBACK_TRY_CATCH_ERROR_IDENTITY_AND_K3_ABI). Exact exit: admit a narrow synchronous callback capability invoked exactly once with absence, literal true, all other returns and thrown/apply-trap failures mapped exactly to typed K3, preserving effects and order.

Minimum vectors: undefined, true/false/0/1/Object/Promise, ordinary throw, Proxy apply trap, mutation and re-entry.
