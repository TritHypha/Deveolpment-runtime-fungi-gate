# Slice 660 combineAll Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#combineAll`.

`BLOCKED` (BLOCKED_BY_REPEATED_LIVE_COLLECTION_TRAVERSAL_AND_UNBOUNDED_ARRAY_ABI): Use one immutable bounded collection snapshot with exact ordered fold/value/provenance traversal and physical collection proof.

Minimum vectors: Empty/single/N; sparse/holes/proxy/overridden map; mutation between three passes; huge input; singleton invalid trust.
