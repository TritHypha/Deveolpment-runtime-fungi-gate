# SLIDE R1 Preflight Evidence

- **Date:** 2026-07-29
- **Scope:** first-fixture export-shape policy only
- **Implementation:** `packages-galerina/galerina-core-compiler/src/self-hosted/slide-r1-preflight.fungi`
- **Test:** `packages-galerina/galerina-core-compiler/tests/slide-r1-preflight.test.mjs`
- **Result:** 20/20 focused tests pass

## What now exists

The `.fungi` flow `preflightSLIDER1` checks the exact proposed
`slide_k3_checked_add_v1` fact set. It uses total `match` statements rather than
repeated `if` chains. Every mismatch returns `Verdict.Deny`, `REFUSED`, and one
ordered failure identity:

| Identity | Refusal |
|---|---|
| `SLIDE-R1-EXPORT-001` | fixture identity outside the frozen slice |
| `SLIDE-R1-EXPORT-002` | qualifier is not `pure` |
| `SLIDE-R1-EXPORT-003` | parameter count is not three |
| `SLIDE-R1-EXPORT-004` | left parameter is not `Int32` |
| `SLIDE-R1-EXPORT-005` | right parameter is not `Int32` |
| `SLIDE-R1-EXPORT-006` | admission parameter is not `Verdict` |
| `SLIDE-R1-EXPORT-007` | result type is not the closed fixture result |
| `SLIDE-R1-EXPORT-008` | K3 control is not exhaustive |
| `SLIDE-R1-EXPORT-009` | checked `Int32` arithmetic is absent |
| `SLIDE-R1-EXPORT-010` | terminal control flow is incomplete |
| `SLIDE-R1-EXPORT-011` | executable body is incomplete or missing |
| `SLIDE-R1-EXPORT-012` | execution still depends on the source AST |
| `SLIDE-R1-EXPORT-013` | effects are present in the effect-free R1 slice |
| `SLIDE-R1-EXPORT-014` | host handles enter the no-address value profile |
| `SLIDE-R1-EXPORT-015` | declared failure set is not the required closed set |
...
`SLIDE-R1-EXPORT-011`; it is never assumed.

## Authority boundary

This kernel is not SLIDE admission authority. Today its request is constructed
...
5. keep the existing WAT identity/default/walker paths outside the SLIDE path.

The `SLIDE-R1-EXPORT-*` names are stable within this preflight contract. They
...
node galerina.mjs check \
  packages-galerina/galerina-core-compiler/src/self-hosted/slide-r1-preflight.fungi \
  --strict-types
  PASS: 0 errors, 0 governance warnings

node --test \
  packages-galerina/galerina-core-compiler/tests/slide-r1-preflight.test.mjs
  PASS: 20 tests
...
- SLIDE object generation, packaging, admission, or execution.
