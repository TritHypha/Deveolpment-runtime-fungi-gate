# Slice 753 src/key-rotation.ts#verifyRing Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#verifyRing`.

`BLOCKED` (BLOCKED_BY_MUTABLE_RING_SET_REGEX_HMAC_TRY_CATCH_AND_ALIAS_ABI). Exact exit: capture one inert ring/key snapshot, bound dense epochs, close every field/status/kind, preserve Set/case/regex/safe-number/HMAC semantics and source-equivalent false-versus-throw behavior, then physically prove the whole decision.

Minimum vectors: wrong class, hostile getters before try, sparse/mutating epochs, duplicate IDs/commits/files, status/count gaps, malformed head, forged MAC and detached/SAB key.
