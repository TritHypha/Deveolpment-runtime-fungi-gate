# Slice 682 CLASS_ORDER Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#CLASS_ORDER`.

`BLOCKED` (BLOCKED_BY_RECORD_CONSTANT_AND_CLOSED_CLASSIFICATION_PHYSICAL_ABI): Admit an exact closed three-tag table with physical bounds and reject every surplus selector.

Minimum vectors: Exact 0/1/2 mapping; all pairs; invalid selectors; mutation guard.
