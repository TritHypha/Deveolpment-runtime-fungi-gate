# Slice 752 src/key-rotation.ts#createKeyRing Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#createKeyRing`.

`BLOCKED` (BLOCKED_BY_SECRET_TYPED_ARRAY_RECORD_BORDER_HMAC_ERROR_IDENTITY_AND_MUTABLE_RING_ABI). Exact exit: decode genesis exactly once as own data, close kind/commit/tick domains, bind key custody and HMAC bytes, preserve typed Error exits and Option retiredTick, and return an immutable non-aliasing ring or admit live identity physically.

Minimum vectors: missing/weak/detached/SAB keys, split getters, malformed commits, empty IDs, tick extrema, every Error path and post-return mutation.
