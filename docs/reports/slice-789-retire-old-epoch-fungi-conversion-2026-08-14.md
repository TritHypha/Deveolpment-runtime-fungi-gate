# Slice 789 src/key-rotation.ts#retireOldEpoch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#retireOldEpoch`.

`BLOCKED` (BLOCKED_BY_IRREVERSIBLE_RETIRE_POLICY_TYPED_ARRAY_CALLBACK_THROW_AND_RING_INVARIANT_ABI). Exact exit: bind custody destruction and state retirement in a prepare/commit receipt or rollback-safe protocol; capture ring/key/policy once; preserve source precedence in which active-epoch lookup and `retireVerdict` execute before `closePhase` checks `drained`, unless an owner-approved versioned change migrates it; replace throw with typed outcomes and prove irreversible ordering physically.

Minimum vectors: wrong phase combined with retire denial, crypto failure and live-ring mutation; no/multiple active; every policy/revocation mode; key/tick states; callback throw; markRevoked failure; epoch vanished; and destruction-before-refusal.
