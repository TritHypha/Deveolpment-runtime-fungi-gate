# Slice 661 isTrusted Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#isTrusted`.

`BLOCKED` (BLOCKED_BY_FORGEABLE_TAG_ONLY_TRUST_GUARD): Authenticate a closed own-data Epistemic record with provenance-bound Hallmark/receipt; a caller tag alone cannot mint Trusted.

Minimum vectors: Forged trust=1; missing/surplus/inherited/accessor/proxy; absent payload/provenance; post-check mutation; singleton tamper.
