# Slice 620 CAP_MLDSA_CONTEXT Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/capability-grant.ts#CAP_MLDSA_CONTEXT`.

`BLOCKED` (BLOCKED_BY_FIXED_BYTE_ARRAY_CRYPTO_CONTEXT_ABI): Require immutable copied context bytes, exact ML-DSA suite identity and physical crypto receipt.

Minimum vectors: exact bytes; omitted/empty/single-byte mutation; cross-surface substitution; retained-alias mutation.
