# Slice 762 src/key-rotation.ts#GateResult Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#GateResult`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public Verdict plus readonly reasons declaration until a governed binding generator preserves it for every consumer.

Minimum vectors: all Verdict tags, empty/multiple reasons, alias mutation and declaration compatibility.
