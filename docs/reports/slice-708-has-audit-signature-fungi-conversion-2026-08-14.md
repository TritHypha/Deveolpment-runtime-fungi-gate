# Slice 708 governance-enforcer.ts#GovernanceEnforcer.hasAuditSignature Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer.hasAuditSignature`.

`BLOCKED` (BLOCKED_BY_STRING_PRESENCE_MASQUERADING_AS_CRYPTOGRAPHIC_AUTHORITY). Exact exit: query an independently verified, freshness-bound signature receipt for the current action/input/policy rather than string presence, with stale, replayed, reset and overwritten states refused.

Minimum vectors: Arbitrary empty stale and replayed Strings.
