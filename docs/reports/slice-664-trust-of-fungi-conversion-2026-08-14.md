# Slice 664 trustOf Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#trustOf`.

`BLOCKED` (BLOCKED_BY_CALLER_MINTABLE_ACTIVE_TRUST_PROPERTY_AND_PHYSICAL_BRAND_ABI): Decode one exact own-data snapshot with provenance-bound Trust identity and reject every non-trit before physical use.

Minimum vectors: Three trits; -2/2/NaN/infinities/String/BigInt; missing/inherited/getter/proxy; forged +1.
