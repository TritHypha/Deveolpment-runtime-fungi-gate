# Slice 769 src/key-rotation.ts#tripleLockVerdict Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#tripleLockVerdict`.

`BLOCKED` (BLOCKED_BY_GATE_RESULT_RECORD_REASON_ARRAY_AND_K3_FOLD_PHYSICAL_ABI). Exact exit: admit exact GateResult/reasons records, independently minted Verdicts, allOf and concatenation order through the physical profile.

Minimum vectors: all 27 K3 combinations, invalid erased verdicts, reason getters/proxies/throws/order and alias mutation.
