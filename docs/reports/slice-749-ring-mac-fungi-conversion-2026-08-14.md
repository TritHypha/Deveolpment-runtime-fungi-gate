# Slice 749 src/key-rotation.ts#ringMac Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#ringMac`.

`BLOCKED` (BLOCKED_BY_NODE_HMAC_SECRET_TYPED_ARRAY_AND_CANONICAL_PREIMAGE_ABI). Exact exit: admit content-bound HMAC-SHA-256 with explicit secret-key custody, exact TypedArray view/copy policy, UTF-8 domain framing, lowercase hex and typed provider failures from one immutable snapshot, then prove a physical receipt.

Minimum vectors: empty/zero/short keys, offset/detached/resized/SAB views, Unicode/lone surrogate preimages, provider failure and mutation during hashing.
