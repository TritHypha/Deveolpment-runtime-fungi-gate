# Slice 818 src/plugin-manifest.ts#canonicalPluginManifestString Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/plugin-manifest.ts#canonicalPluginManifestString`.

`BLOCKED` (BLOCKED_BY_PLUGIN_METADATA_ADMISSION_AND_CANONICAL_STRING_ABI). Exact exit: derive canonical bytes from one admitted metadata snapshot, never a reread; bind exact JSON String production, UTF-8 bytes and typed refusal before signing or hashing.

Minimum vectors: all envelope and canonical snapshot vectors; mutation between admission/stringification; serializer undefined/throw; Unicode byte KAT.
