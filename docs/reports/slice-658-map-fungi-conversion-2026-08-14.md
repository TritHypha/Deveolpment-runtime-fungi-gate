# Slice 658 map Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#map`.

`BLOCKED` (BLOCKED_BY_EFFECTFUL_CALLBACK_AND_SPLIT_SNAPSHOT): Capture the input once or admit an effect-qualified callback with typed failure/order, immutable output and physical ABI.

Minimum vectors: Every trust; callback return/throw/mutation; getter/proxy; output alias and provenance order.
