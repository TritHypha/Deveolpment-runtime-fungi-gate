# Slice 919 src/substrate-erasure.ts#effectiveEraseModel Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#effectiveEraseModel`.

`BLOCKED` (BLOCKED_BY_CALLER_MINTABLE_ATTESTED_FLAG_ACTIVE_RECORD_SPLIT_READ_AND_ERASE_MODEL_ABI). Exact exit: capture one exact own-data descriptor and replace caller-mintable `attested` with a provenance-bound Hallmark or verifier receipt tied to id/model, then prove the closed physical EraseModel ABI and switch consumers.

Minimum vectors: undefined/null/wrong class; missing/undefined/inherited/accessor/proxy; false/true/1; both models/surplus/case; split mutation; forged attested overwrite; getter throw.
