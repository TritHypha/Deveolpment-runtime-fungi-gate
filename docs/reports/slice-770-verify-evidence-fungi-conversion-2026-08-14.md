# Slice 770 src/key-rotation.ts#VerifyEvidence Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#VerifyEvidence`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public declaration until governed bindings preserve its numeric/String surface and callback consumers.

Minimum vectors: missing/surplus fields, number boundaries, head strings and declaration compatibility.
