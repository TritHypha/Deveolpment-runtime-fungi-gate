# Slice 903 src/registry-public-verifier.ts#createRegistryPublicVerifiers.mlDsa65 Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/registry-public-verifier.ts#createRegistryPublicVerifiers.mlDsa65`.

`BLOCKED` (BLOCKED_BY_ML_DSA65_CRYPTO_RECEIPT_CONTEXT_MESSAGE_BYTEARRAY_SIGNATURE_PROFILE_ERROR_COLLAPSE_AND_RESULT_ABI). Exact exit: bind exact ML-DSA-65 public key, canonical signature, message, role context and keyId in an independently admitted receipt, preserving no-key short circuit and false-on-parser/verifier failure.

Minimum vectors: all role contexts/cross-role replay; matching/mismatching keyId; valid/tampered/wrong-length/noncanonical signature; invalid key; message/key/context alias/detach/SAB mutation; verifier false/throw/version drift.
