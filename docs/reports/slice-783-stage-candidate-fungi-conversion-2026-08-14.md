# Slice 783 src/key-rotation.ts#stageCandidate Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#stageCandidate`.

`BLOCKED` (BLOCKED_BY_LIVE_RING_TYPED_ARRAY_EXCEPTION_CALLBACK_AND_PHASE_TRANSITION_ABI). Exact exit: bind live ring, key view, candidate and caught Error behavior to one snapshot; preserve source precedence in which `stageEpoch` and its caught failure execute before `closePhase` checks `ready`, unless an owner-approved versioned change migrates that observable order; close the typed-array policy and physically prove the transition.

Minimum vectors: wrong phase combined with stage Error and non-Error failure; invalid candidate; weak/detached/SAB key; duplicate fields; callback mutation; and original identity.
