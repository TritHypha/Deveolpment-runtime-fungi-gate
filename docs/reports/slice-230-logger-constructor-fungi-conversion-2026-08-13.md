# Slice 230 Logger.constructor Fungi conversion adjudication

## Outcome

`Logger.constructor` is `BLOCKED_BY_MUTABLE_LOGGER_SINK_CLOCK_CALLBACK_ABI`. It creates active state, retains sink/clock/base-field identities and normalises caller keys. No placeholder Fungi asset is created.

Required exit: Admit the complete active logger object, lifecycle, closed inputs, retained capabilities and mutable failure state.
