# Slice 235 Logger.child Fungi conversion adjudication

## Outcome

`Logger.child` is `BLOCKED_BY_SHARED_SINK_CLOCK_CHILD_LOGGER_CAPABILITY_ABI`. It creates a new logger while retaining sink/clock identities, merging open fields and resetting local failure state. No placeholder Fungi asset is created.

Required exit: Define child capability derivation, lease sharing, field snapshots, naming and counter ownership.
