# Slice 787 src/key-rotation.ts#fallbackToOldEpoch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#fallbackToOldEpoch`.

`BLOCKED` (BLOCKED_BY_FALLBACK_RING_TYPED_ARRAY_EXCEPTION_CALLBACK_AND_REVERSAL_ORDER_ABI). Exact exit: admit the fallback ring/key/tick snapshot and typed errors, enforce candidate revoke plus predecessor reactivate atomically, and prove diagnostic/log/identity order physically.

Minimum vectors: all wrong phases, missing/duplicate epochs, key view states, tick extrema, Error/non-Error, callback throw, null transition and log order.
