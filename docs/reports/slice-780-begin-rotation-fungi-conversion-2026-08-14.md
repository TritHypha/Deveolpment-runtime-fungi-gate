# Slice 780 src/key-rotation.ts#beginRotation Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#beginRotation`.

`BLOCKED` (BLOCKED_BY_ACTIVE_RING_ALIAS_REFERENCE_IDENTITY_AND_ROTATION_PROCESS_STATE_ABI). Exact exit: admit exact active ring/alias identity or owner-approved copy/freeze semantics, preserving retained nested mutation, Option transition, log identity and physical process state.

Minimum vectors: top-level/nested mutation after call, proxy/accessor ring, repeated identity and wrong-class input.
