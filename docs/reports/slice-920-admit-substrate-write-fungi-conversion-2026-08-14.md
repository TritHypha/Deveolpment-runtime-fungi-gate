# Slice 920 src/substrate-erasure.ts#admitSubstrateWrite Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/substrate-erasure.ts#admitSubstrateWrite`.

`BLOCKED` (BLOCKED_BY_FORGED_ATTESTED_AUTHORITY_ACTIVE_PAYLOAD_DESCRIPTOR_K3_DIAGNOSTIC_CALLBACK_REASON_AND_ADMISSION_RECORD_ABI). Exact exit: capture exact payload and provenance-bound descriptor records once; preserve model-first and overwrite short-circuit order, the crypto-only truth table, K3 boundary, diagnostic effects/failures, exact reasons and complete physical admission record.

Minimum vectors: full model×taint×sealed set; absent/missing/surplus/inherited/accessor/proxy; forged attested; overwrite payload non-observation; getter order/split/throw; diagnostic absent/present/throw/reenter; record consistency.
