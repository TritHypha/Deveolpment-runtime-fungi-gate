# Slice 709 governance-enforcer.ts#GovernanceEnforcer.checkTransition Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer.checkTransition`.

`BLOCKED` (BLOCKED_BY_MUTABLE_POLICY_ALIAS_CALLER_MINTED_PRECONDITIONS_AND_FAIL_OPEN_DEFAULT). Exact exit: decode branded closed Trit/Verdict inputs, retain one immutable policy snapshot, require signed/schema receipts bound to the same action, make every unknown requirement deny/refuse, preserve first-match and requirement order, and physically prove the transition result.

Minimum vectors: Full trit matrix NaN fractions unknown requirements and partial prerequisites.
