# Slice 764 src/key-rotation.ts#ReadinessEvidence Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#ReadinessEvidence`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the three-number public declaration until governed bindings preserve field names, readonly surface and consumers.

Minimum vectors: missing/surplus fields and every binary64 boundary for the eventual runtime decoder.
