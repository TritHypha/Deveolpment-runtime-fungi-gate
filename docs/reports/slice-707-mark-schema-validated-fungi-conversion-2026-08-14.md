# Slice 707 governance-enforcer.ts#GovernanceEnforcer.markSchemaValidated Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#GovernanceEnforcer.markSchemaValidated`.

`BLOCKED` (BLOCKED_BY_CALLER_MINTABLE_SCHEMA_VALIDATED_BOOLEAN). Exact exit: replace the public Boolean mint with a provenance-bound validator Hallmark/receipt tied to one immutable input, exact schema/version and action identity, including forgery, replay and cross-input refusal.

Minimum vectors: Call without validation and validate A then authorize B.
