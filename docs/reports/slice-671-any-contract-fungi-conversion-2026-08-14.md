# Slice 671 anyContract Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#anyContract`.

`BLOCKED` (BLOCKED_BY_RETAINED_ACTIVE_CALLBACK_ARRAY_ALIAS_K3_OR_AUTHORITY_ABI): Apply the exact closed callback/fold boundary and prove every OR clause is independently sufficient authority.

Minimum vectors: Empty/single/multiple K3; invalid singleton; array mutation/proxy; callback throw/order; insufficient-alternative negatives.
