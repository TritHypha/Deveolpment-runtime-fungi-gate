# Slice 754 src/key-rotation.ts#activeEpoch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#activeEpoch`.

`BLOCKED` (BLOCKED_BY_LIVE_ARRAY_FILTER_SIGNING_AUTHORITY_ALIAS_NULL_AND_MUTATION_ABI). Exact exit: admit bounded live filter/property and returned alias identity exactly, or migrate all consumers to one verified ring snapshot and Option of immutable signing capability with physical authority proof.

Minimum vectors: zero/one/multiple active, holes, inherited/accessor/proxy status, mutation during filter and returned-epoch mutation.
