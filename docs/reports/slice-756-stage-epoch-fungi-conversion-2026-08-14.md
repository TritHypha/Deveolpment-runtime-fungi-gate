# Slice 756 src/key-rotation.ts#stageEpoch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#stageEpoch`.

`BLOCKED` (BLOCKED_BY_RING_CRYPTO_MUTABLE_ARRAY_ALIAS_THROW_AND_TRANSITION_ABI). Exact exit: capture ring, key view and candidate once; verify and derive the new HMAC from that snapshot; preserve collision scans and distinct Errors; adopt immutable epochs or physically admit retained identities.

Minimum vectors: invalid ring, staged duplicate, malformed candidate, key/file/commit collisions, tick extrema, split getters, key mutation and old/new alias mutation.
