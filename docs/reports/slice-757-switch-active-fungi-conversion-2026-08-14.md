# Slice 757 src/key-rotation.ts#switchActive Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#switchActive`.

`BLOCKED` (BLOCKED_BY_RING_CRYPTO_ACTIVE_EPOCH_TRANSITION_THROW_AND_NULL_ABI). Exact exit: use one verified ring/key/tick snapshot, enforce exactly one active and staged epoch, preserve safe-tick and distinct Error exits, define unchanged-epoch alias policy and MAC the same derived array.

Minimum vectors: invalid ring, zero/multiple active or staged, invalid/backward tick, getter mutation, detached key and unchanged alias mutation.
