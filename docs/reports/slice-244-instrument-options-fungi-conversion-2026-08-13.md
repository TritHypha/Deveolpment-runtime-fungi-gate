# Slice 244 InstrumentOptions Fungi conversion adjudication

## Outcome

`InstrumentOptions` is `NO_RUNTIME_BEHAVIOR`. The erased declaration validates no clock callback or timing authority. No placeholder Fungi asset is created.

Required exit: Charge callback behavior to instrumentDispatch and safeNow; the declaration grants no runtime authority.
