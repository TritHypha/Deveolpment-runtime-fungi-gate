# Slice 702 governance-enforcer.ts#TPL_DEFAULT_POLICY Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/governance-enforcer.ts#TPL_DEFAULT_POLICY`.

`BLOCKED` (BLOCKED_BY_EXPORTED_MUTABLE_TRANSITION_POLICY_SINGLETON). Exact exit: obtain an owner-approved versioned copy/freeze contract or admit the exact active singleton identity, close Trit/Verdict and requirement tags, make the default deny-only, and prove mutation/identity parity plus the physical chain.

Minimum vectors: Push remove reorder and mutate defaultAction.
