# Slice 904 src/snapshot-key-provider.ts#SNAPSHOT_KEY_CONTEXT Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/snapshot-key-provider.ts#SNAPSHOT_KEY_CONTEXT`.

`CANDIDATE` (CANDIDATE_PRIMITIVE_STRING_CONTEXT_REQUIRES_PHYSICAL_PROOF). Exact exit: bind the exact UTF-8 bytes of `galerina.snapshot.epoch.key.v1`, including its distinction from the appended NUL, through Fungi, GIR, physical `.slide`, independent re-admission and VOK before any consumer or retirement claim.

Minimum vectors: exact bytes; one-byte mutation; case/punctuation; empty; embedded/trailing NUL; cross-domain replay.
