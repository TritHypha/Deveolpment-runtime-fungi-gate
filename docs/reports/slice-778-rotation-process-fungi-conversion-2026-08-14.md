# Slice 778 src/key-rotation.ts#RotationProcess Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#RotationProcess`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION). Exact exit: retain and version the public process declaration until governed bindings preserve phase, ring, transition and log contracts.

Minimum vectors: every phase, null/present transition, ring/log aliases and missing/surplus fields.
