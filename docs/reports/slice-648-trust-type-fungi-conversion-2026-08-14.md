# Slice 648 Trust type alias Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/epistemic-type-state.ts#Trust.type`.

`NO_RUNTIME_BEHAVIOR` (PUBLIC_DTS_DECLARATION): Retain/version the exact public Verdict alias and every declaration consumer; an erased alias owns no runtime authority.

Minimum vectors: Exact -1/0/+1 alias and assignability; external declaration compatibility.
