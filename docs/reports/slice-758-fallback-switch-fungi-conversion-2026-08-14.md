# Slice 758 src/key-rotation.ts#fallbackSwitch Fungi conversion adjudication

Scope: `packages-galerina/galerina-tower-citizen/src/key-rotation.ts#fallbackSwitch`.

`BLOCKED` (BLOCKED_BY_RING_CRYPTO_REVERSAL_ALIAS_THROW_AND_NULL_ABI). Exact exit: capture and verify one ring/key/tick snapshot, require the exact active/predecessor-retired pair, preserve retiredTick default and typed errors, and physically prove the revoke/reactivate reversal.

Minimum vectors: missing active/predecessor, null/zero/nonfinite/backward retiredTick, bad tick, proxy mutation, output alias mutation and HMAC failure.
