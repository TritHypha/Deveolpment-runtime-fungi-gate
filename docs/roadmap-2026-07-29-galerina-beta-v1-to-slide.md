...
| Production custody and artifact activation | 🟥 | The least-authority custody contract, disposable executor, generic fail-closed durability seam, fifteen-boundary simulator and closed native-adapter descriptor/host gate exist. A zero-dependency Windows host probe passes 4/4. A separate non-executing artifact inspector passes 7/7, binds fixed-path single-link bytes to PE/ELF/Mach-O architecture plus SHA-256, and has a checker-clean `.fungi` terminal fold. Both are non-authorizing. The production digest list remains empty. Release remains blocked on actual content-bound N-API export/loading proof, retained-handle adapters for Windows 10/11, Linux-family and macOS, and real crash/power-loss evidence. No owner-key operation is required for that engineering evidence |
