...
| Production custody and artifact activation | 🟥 | The least-authority custody contract, disposable executor, generic fail-closed durability seam, fifteen-boundary simulator and closed native-adapter descriptor/host gate exist. The zero-dependency Windows candidate is 5/5: host refusal plus a live native directory `FlushFileBuffers` call succeeds on this Windows 10 NTFS host. A separate non-executing artifact inspector passes 7/7, binds fixed-path single-link bytes to PE/ELF/Mach-O architecture plus SHA-256, and has a checker-clean `.fungi` terminal fold. All remain non-authorizing. The production digest list remains empty. Release remains blocked on actual content-bound loading, complete publication ordering, Windows 10/11 + Linux + macOS crash matrices and power-loss evidence |
