...
target selection belong in `packages-ts/galerina-core-compute/`. Target-specific planning
belongs in `packages-ts/galerina-target-cpu/`, `packages-ts/galerina-target-native/`,
`packages-ts/galerina-target-gpu/` and `packages-ts/galerina-target-photonic/`. Optimized CPU
kernel contracts belong in `packages-ts/galerina-cpu-kernels/`. AI inference contracts
belong in `packages-ts/galerina-ai/`, with low-bit and ternary AI backend planning in
