...
in `packages-ts/galerina-core-compute/`. Target-specific output belongs in target packages
such as `packages-ts/galerina-target-native/`, `packages-ts/galerina-target-wasm/`,
`packages-ts/galerina-target-gpu/` and `packages-ts/galerina-target-photonic/`.
