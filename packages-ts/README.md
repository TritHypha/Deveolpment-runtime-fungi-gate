...
`packages-ts/` is the home for active reusable Galerina packages in this beta
...
packages-ts/    Galerina package collection and beta Galerina package experiments
```
...
|-- packages-ts/       Galerina packages, optionally a nested Git repository
|-- boot.fungi
`-- main.fungi
```

`packages-ts/` may later become its own Git repository or submodule. If a
