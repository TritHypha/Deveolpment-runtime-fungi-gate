...
/** Walk up from this file until a directory containing `packages-ts` is
...
function locateRoot(startDir) {
  let dir = startDir;
  for (let i = 0; i < 12; i++) {
    if (fs.existsSync(path.join(dir, "packages-ts"))) return dir;
...
if (!ROOT) {
  console.error("galerina-hypha: could not locate the repository root (no ancestor directory contains `packages-ts`).");
...
const COMPILER_DIST = path.join(ROOT, "packages-ts", "galerina-core-compiler", "dist");
