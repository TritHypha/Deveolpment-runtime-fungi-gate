...
| **Nothing to find** | The repo root is located by walking up from the script until a directory containing `packages-ts` is found. No env var, no cwd assumption, nothing to configure. `--root` overrides the search when the package has been relocated — an escape hatch, not a setup step. |
