...
async function fixture() {
  ...
  await write(root, "galerina.workspace.json", JSON.stringify({
    packages: packages.map((name) => `packages-ts/${name}`),
    compilerPackage: "packages-ts/galerina-a",
  }));
  await write(root, "packages-ts/galerina-a/package.json", JSON.stringify({
    name: "@galerina/a",
    scripts: { test: "node --test tests/*.test.mjs" },
  }));
  await write(root, "packages-ts/galerina-b/package.json", JSON.stringify({
    name: "@galerina/b",
    dependencies: { "@galerina/a": "workspace:*" },
    scripts: { test: "node --test tests/*.test.mjs" },
  }));
  await write(root, "packages-ts/galerina-c/package.json", JSON.stringify({
    name: "@galerina/c",
...
test("package change expands the exact reverse dependency closure", async () => {
  ...
  const plan = buildImpactPlan({
    root,
    changedPaths: ["packages-ts/galerina-b/src/b.mjs"],
  });
...
test("compiler and package-manifest changes require a full scan", async () => {
  ...
  const compiler = buildImpactPlan({
    root,
    changedPaths: ["packages-ts/galerina-a/src/compiler.mjs"],
  });
  ...
  const manifest = buildImpactPlan({
    root,
    changedPaths: ["packages-ts/galerina-b/package.json"],
  });
...
test("unknown root paths and malformed paths fail closed to full", async () => {
  const root = await fixture();
  for (const changedPath of ["scripts/unknown-tool.mjs", "../escape.mjs", "packages-ts/not-declared/x.mjs"]) {
