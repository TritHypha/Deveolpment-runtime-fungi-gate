...
**Verified state (2026-08-02):** the current reviewed tree passes 98/98
packages, 8,781 tests, strict phase-close 84/84, exhaustive 85/85, graph 5/5;
the security gate reports 31 files with 0 findings and 0 errors. A later commit
or documentation-only change is not automatically release evidence: an exact
clean commit must still be named by its platform receipt. Counts are generated by
`run-all-tests.cjs --emit-counts`; this statement is evidence, not production
authority.
